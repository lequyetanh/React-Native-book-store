export let lich_su = 
[
  {
    id: 474,
    bookName: 'QUỐC SẮC SINH KIÊU',
    bookCover: 'https://img.dtruyen.com/public/images/large/quocsacsinhkieu61MUsiZFNF.jpg',
    rating: '7.6',
    language: 'Việt Nam',
    pageNo: 1057,
    author: 'Sa Mạc',
    genre: 'lịch sử',
    content: '',
    readed: 39,
    description: [
      'Bạn đang đọc truyện Quốc Sắc Sinh Kiêu của tác giả Sa Mạc trên trang đọc truyện online.',
      'Lục Long tụ binh, Bồ Tát mở cửa!Một bẫy rập quỷ dị lớn động trời, trong cục có cục, trong kế có kế, chân thật và giả dối, ai là người trong kế, ai là kẻ sắp đặt? Sau sấm ngôn tám chữ kia, chuyện cũ phủ đầy bụi lại che giấu cái gì? Là sói tuần thiên hạ, hay là cáo giảo hoạt giấu diếm thế gian?Từng khúc hát ca ngợi tráng sĩ chưa xong, từng chiếc màn đã vén lên lộ bóng giai nhân!',
      '…….',
      'Sa Mạc giới thiệu chỉ có vậy, chẳng biết có gợi lên trong độc giả những tò mò háo hức, những kịch tính hấp dẫn như biết bao ám ảnh từ Quyền Thần một thời hay không nữa? Chỉ biết rằng đó là truyện của Sa Mạc viết, và thế cũng đủ, để thanhco em vốn đã tính quy ẩn sau khi khép lại Quyền Thần tiếp tục len lén thức đêm len lén mò mặt dậy từ tơm tởm sáng lóc cóc dịch. Truyện mới post được mấy chục chương, thường thì rất ít dịch giả nhận dịch, sợ tác giả nửa đường vỡ đê vào viện sinh con phải ngừng viết thì mem cũng bỏ, lại nữa, nhỡ truyện không hay sẽ ít người đọc.... Nhưng kệ, em vẫn rủ rê koba, rồi đi nằn nì bqtmetruyen cho phép KoCo vừa dịch vừa bám theo tác giả. Có thể sẽ chẳng có ai thèm đọc. Có thể mem sẽ chờ hòm hòm một chút rồi mới đọc. Thậm chí, đợi full rồi mới đọc. Không loại trừ tình huống điểm không đủ để em trả tiền điện ngồi dịch. Nhưng em vẫn cứ … kệ. Vì em tin vào uy tín của Sa Mạc. Vì em tin vào sức viết của lão đã được kiểm chứng qua Quyền Thần. Vì cơ bản, em cần được đọc. Một cuốn sách khiến em ngộp thở. Một cuốn sách khiến em rơi lệ. Một cuốn sách để hâm nóng trong tâm hồn già nua của em những ngọn lửa.',
      'Có vẻ là một lời giới thiệu truyện hơi cá nhân. Nhưng cũng cứ… kệ. Hihi. Bạn đọc mấy dòng này và hiểu, thì quý. Bạn next và xì môi, KoCo chúng em cũng xin cảm tạ. Tùy duyên thôi vậy! Cho dù sau này số phận của Quốc Sắc Sinh Kiêu thế nào, thì hai thằng em cũng đã đặt buồn vui của mình vào đó rồi.',
      'Cho em riêng tư nốt câu này. Cám ơn bqt đã luôn hóng tìm truyện mới cho em! Cám ơn Koba lúc nào cũng nhẫn nại chịu đựng mọi ngẫu hứng bất thường của em! Và riêng cám ơn tất cả mọi người! Chỉ cần bạn đọc lời giới thiệu viết vội của Quốc Sắc Sinh Kiêu này thôi, bạn cũng là tâm giao tri kỷ của KoCo rồi. Huống hồ chi còn cùng KoCo đi suốt chặng đường dài hàng nghìn chương tít tắp sau nữa…',
      'Lắm mồm thật, thôi chúng ta cùng vén bức màn bí mật đầu tiên, ngay từ trang đầu tiên của Quốc Sắc Sinh Kiêu cho sớm chợ!',
      '(cái này gọi là đổi tone giọng từ la thứ sang đô trưởng)'
    ],
    view: '3,441,793',
    date: '14:21:54 10/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Cô Là Nô Lệ Của Ta',
      'Chương 2 - Hít Thở Không Thông',
      'Chương 3 - Cô Càng Thống Khổ, Ta Càng Vui',
      'Chương 4 - Điên Rồi!',
      'Chương 5 - Sống Không Bằng Chết',
      'Chương 6 - Liên Luỵ',
      'Chương 7 - Sợ Hãi',
      'Chương 8 - Sinh Con Cho Bổn Toạ',
      'Chương 9 - Diệt Tộc',
      'Chương 10 - Tra Tấn',
      'Chương 11 - Bất Lực',
      'Chương 12 - Dục Vọng',
      'Chương 13 - Hầm Mộ Cổ',
      'Chương 14 - Thăm Dò',
      'Chương 15 - Mặc Ca Ca',
      'Chương 16 - Dâm Loạn',
      'Chương 17 - Chứng Cứ',
      'Chương 18 - Tôi Tin Anh',
      'Chương 19 - Một Chữ Cũng Không Tin',
      'Chương 20 - Cứu Nam Nhân',
      'Chương 21 - Tịch Liễu Căng Thẳng',
      'Chương 22 - Huynh Yêu Ta Đến Nhường Nào',
      'Chương 23 - Cô Phải Chết',
      'Chương 24 - Biến Mất',
      'Chương 25 - Người Nam Nhân Kia',
      'Chương 26 - Thả Tự Do Cho Nàng',
      'Chương 27 - Người Tôi Yêu Vẫn Luôn Là Anh',
      'Chương 28 - Không Tiếc Bất Cứ Giá Nào',
      'Chương 29 - Đã Quên',
      'Chương 30 - Quấy Nhiễu Đêm Động Phòng'
    ]
  },
  {
    id: 475,
    bookName: 'THIẾT HUYẾT ĐẠI MINH',
    bookCover: 'https://img.dtruyen.com/public/images/large/thiethuyetdaiminhouzhMdunqL.jpg',
    rating: '5.6',
    language: 'Việt Nam',
    pageNo: 1139,
    author: 'Tịch Mịch Kiếm Khách',
    genre: 'lịch sử',
    content: '',
    readed: 11,
    description: [
      'Thiết Huyết Đại Minh là truyện đô thị nói về nhân vật chính Vương Phác kiếp trước là một thanh niên vừa tốt nghiệp đại học, vì không xin được việc nên đi làm đại ca đường phố, cầm đầu một nhóm du côn, trong một lần chém giết tranh giành địa bàn, kết quả Vương Phác bị trúng một gậy vào đầu ngất xỉu, linh hồn hắn xuyên về triều Đại Minh, thời kỳ Sùng Trinh Đế nắm quyền hồn nhập vào Vương Phác, một Tổng binh trấn Đại Đồng, thiếu gia nhà giàu ăn chơi trác táng, sợ chết, chức vụ có được cũng là do bỏ tiền ra mua. Cũng từ đây, một Vương Phác mới xuất hiện đã hoàn toàn làm thay đổi lịch sử.',
      'Vì tuyệt hậu hoạn, Vương Phác thanh trừ Mãn Thanh Thát Tử. Vì bình biên hoạn, Vương Phác chinh phục cả Đại Mạc. Vì luyện binh, Vương Phác đã dùng mọi thủ đoạn thu về số bạc khổng lồ! Vì kiếm tiền, Vương Phác có thể sử dụng toàn bộ mánh khóe, kết quả không thể không tránh khỏi phát sinh hiểu lầm với Tần Hoài Bát Diễm.',
      'Vì giữ gìn thống nhất đế quốc, Vương Phác tiêu diệt Lý Tự Thành cùng Trương Hiến Trung, lột da rút gân Hán gian Ngô Tam Quế nhét cỏ làm thành đèn lồng da người! .... Mời bạn đọc cùng theo dõi truyện.',
      ''
    ],
    view: '708,455',
    date: '14:56:40 10/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - CƯỠNG HIẾP CẢ THÀNH PHỐ',
      'Chương 2 - NGƯỜI BỊ ĐÁNH DẤU',
      'Chương 3 - NỖI SỢ HÃI',
      'Chương 4 - QUỶ HÚT MÁU',
      'Chương 5 - BÁC SĨ',
      'Chương 6 - MÊ LỰC CỦA MÁU',
      'Chương 7 - CỐ ĐỂ QUÊN',
      'Chương 8 - VUI SƯỚNG HAY KHÔNG',
      'Chương 9 - LỘ DIỆN',
      'Chương 10 - NGÔI SAO 5 CÁNH TRÊN CÁNH CỬA',
      'Chương 11 - THÀNH HỒI ỨC',
      'Chương 12 - VỤ ÁN TẠI BỆNH VIỆN',
      'Chương 13 - BẢN NĂNG',
      'Chương 14 - LỌ HOA CỦA GRAYSON PERRY',
      'Chương 15 - MÊ HỒN TRẬN',
      'Chương 16 - SÁT THỦ CON SỐ',
      'Chương 17 - LỢN',
      'Chương 18 - THE YORKSHINE RIPPER',
      'Chương 19 - TÌNH YÊU LÀ GÌ',
      'Chương 20 - MÈO VÀ CHUỘT',
      'Chương 21 - 3+1+3',
      'Chương 22 - MÈO VÀ CHUỘT (2)',
      'Chương 23 - ĐÊM NOEL',
      'Chương 24 - ĐƯỜNG BƠI SỐ 6',
      'Chương 25 - PHÓNG 304',
      'Chương 26 - SƯ HUYNH',
      'Chương 27 - ĐẠI HIỆP SÔNG HÔ LAN',
      'Chương 28 - TẦNG TRÊN. ĐỊA NGỤC',
      'Chương 29 - VĨ THANH'
    ]
  },
  {
    id: 476,
    bookName: 'HOÀNG TỘC',
    bookCover: 'https://img.dtruyen.com/public/images/large/hoangtoc3nktmKIkX4.jpg',
    rating: '6.8',
    language: 'Việt Nam',
    pageNo: 1286,
    author: 'Cao Nguyệt',
    genre: 'lịch sử',
    content: '',
    readed: 86,
    description: [
      'Bạn đang đọc truyện Hoàng Tộc của tác giả Cao Nguyệt trên trang đọc truyện online.Hắn - một kẻ lười biếng.Hắn - con của vợ lẻ, thân phận hèn mọn.Che giấu lai lịch, tung hoành thương trường 10 năm bất khả chiến bại.',
      'Đến cuối cùng hắn chỉ là một kẻ lười biếng cô độc, không nơi nương tựa, mang theo truyền kỳ của mình chết đi.Kiếp này, hắn là danh môn hoàng tộc, kiều thê viên mãn.',
      'Thiên hạ phong vân, triều đình tranh đấu, khắp nơi tranh đoạt. Trong thời khắc dầu sôi lửa bỏng, một kẻ lười biếng, đạm mạc danh lợi như hắn phải làm sao, nên đi bước nào?Đến cuối cùng, ai mới là rồng trong loài người?'
    ],
    view: '955,624',
    date: '12:13:42 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Tẩy Trần',
      'Chương 2 - Từ Chối May',
      'Chương 3 - Tìm Mèo',
      'Chương 4 - Vụ Giết Người Tàn Bạo',
      'Chương 5 - Bóng Ma',
      'Chương 6 - Phát Điên',
      'Chương 7 - Giấc Mộng Đêm',
      'Chương 8 - Cõng Ma',
      'Chương 9 - Tiếng Cười',
      'Chương 10 - Thăm Bệnh',
      'Chương 11 - Gọi Hồn',
      'Chương 12 - Đào Mộ',
      'Chương 13 - Mượn Sách',
      'Chương 14 - Ca Băng',
      'Chương 15 - Tòa Cao Ốc',
      'Chương 16 - Nghe Hát Trong Vườn',
      'Chương 17 - Ăn Cắp Vải',
      'Chương 18 - Rung Động',
      'Chương 19 - Tiểu Ngư',
      'Chương 20 - Nguồn Gốc Lời Nguyền',
      'Chương 21 - Thay Đổi',
      'Chương 22 - Trở Về Thành Phố',
      'Chương 23 - Nhập Hồn',
      'Chương 24 - Tiếc Thương Người Đã Khuất',
      'Chương 25 - Chờ Đợi (Hết)'
    ]
  },
  {
    id: 477,
    bookName: 'TIÊU DAO TIỂU THƯ SINH',
    bookCover: 'https://img.dtruyen.com/public/images/large/tieudaotieuthusinhtoIAEeqNAC.jpg',
    rating: '8',
    language: 'Việt Nam',
    pageNo: 699,
    author: 'Vinh Tiểu Vinh',
    genre: 'lịch sử',
    content: '',
    readed: 42,
    description: [
      'Một thanh niên ngành kỹ thuật thế kỷ 21 xuyên không về cổ đại trở thành một thư sinh nghèo. Trong đại não thế mà chứa một cái thư viện, các loại tri thức cái gì cần có đều có! Cùng theo dõi những câu chuyện của chàng thanh niên này qua truyện Tiêu Dao Tiểu Thư Sinh của tác giả Vinh Tiểu Vinh nhé.',
      'Văn án truyện:',
      'Đời này không nói Phong Hầu Bái Tướng, làm sao cũng phải thử cảm giác làm đại quan a?',
      'Đón ánh mặt trời mới mọc, Lý Dịch phát ra ý nguyện.',
      '"Đứng lại!"',
      'Phía trên một con ngựa cao to, một tuyệt sắc nữ tử tinh tế dò xét một phen, nhẹ vẫy tay:',
      '"Trói!"',
      'Đám sơn tặc hung ác sau lưng xông lên. . .',
      'Đêm động phòng hoa chúc sát vách, nhìn thấy tuyệt sắc nữ tử đẩy cửa phòng tiến vào. . . chí nguyện của Lý Dịch phát sinh cải biến.',
      'Nhất phẩm quan nhân tính là gì? Phong Hầu Bái Tướng, không có ý nghĩa! Ta muốn trở thành nam nhân của sơn tặc Vương a!',
      ''
    ],
    view: '1,275,180',
    date: '21:44:30 01/10/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Phu Quân Hung Mãnh (1)',
      'Chương 2 - Phu Quân Hung Mãnh (2)',
      'Chương 3 - Người Lạ Chớ Đến Gần (1)',
      'Chương 4 - Người Lạ Chớ Tới Gần (2)',
      'Chương 5 - Ly Long (*) Huyết Ngọc (1)',
      'Chương 6 - Ly Long Huyết Ngọc (2)',
      'Chương 7 - Ly Long Huyết Ngọc (3)',
      'Chương 8 - Ly Long Huyết Ngọc (4)',
      'Chương 9 - Tăng Ngọc Chương (1)',
      'Chương 10 - Bố Gặp Chuyện',
      'Chương 11 - Chồng Quỷ Ra Tay',
      'Chương 12 - Âm Khí Thấm Vào Xương Tủy',
      'Chương 13 - Tình Nghĩa Bạn Thân',
      'Chương 14 - Đừng Đi Giày Cao Gót',
      'Chương 15 - Bức Tượng Đồng Nhức Mắt',
      'Chương 16 - Tà Đạo Hại Người',
      'Chương 17 - Mặt Quỷ Lượn Lờ',
      'Chương 18 - Một Đêm Ba Thi Thể',
      'Chương 19 - Căn Phòng Chết',
      'Chương 20 - Tà Sư (1)',
      'Chương 21 - Tà Sư (2)',
      'Chương 22 - Sự Phẫn Nộ Lạnh Băng',
      'Chương 23 - Rỉ Thai',
      'Chương 24 - Nữ Quỷ Nhà Giàu (1)',
      'Chương 25 - Nữ Quỷ Nhà Giàu (2)',
      'Chương 26 - Tôi Là Khôn Đạo?',
      'Chương 27 - Tà Linh Bất Diệt',
      'Chương 28 - Lần Đầu Đến Thành Phố Quỷ',
      'Chương 29 - Nơi Tụ Âm Khí Giữ Thi Thể (1)',
      'Chương 30 - Nơi Tụ Âm Khí Giữ Thi Thể (2)'
    ]
  },
  {
    id: 478,
    bookName: 'DẠ THIÊN TỬ',
    bookCover: 'https://img.dtruyen.com/public/images/large/dathientuuIXGj6aaJl.jpg',
    rating: '6.1',
    language: 'Việt Nam',
    pageNo: 1138,
    author: 'Nguyệt Quan',
    genre: 'lịch sử',
    content: '',
    readed: 87,
    description: [
      'Bên cạnh tựa truyện đình đám Đế Quốc Cuồng Lan, Dạ Thiên Sử tuy không nổi trội về bề ngoài nhưng càng chiêm nghiệm càng dễ gây nghiện, càng khai thác bên trong càng nhận ra những điểm hấp dẫn, đặc sắc đầy bất ngờ.',
      '***',
      'Có người nói nơi này là âm tào địa phủ. Loại người như chúng ta đúng là quỷ tốt trong điện Diêm La, nói linh tinh, rõ ràng là linh tinh mà! Đây là người không biết về chúng ta vu oan chúng ta! Loại thành kiến và hiểu lầm này khiến chúng ta mệt nhọc, khiến người tận trung với cương vị công tác vô cùng đau đớn.',
      'Người nói chuyện mặc một bộ đồ nha sai màu xanh nhạt, trên đầu đội mũ vải có vẻ lớn hơn đầu hắn một chút, bên hông thắt một cái đai vải đỏ cổ xưa, dưới chân là một đôi giày ống bề ngoài đen trắng không quá vừa chân, trông rõ ràng là một tên lính canh ngục.....',
      '***',
      'Hắn vốn dòng dõi quan chức nhưng không phải vương hầu, xuất thân thế gia nhưng lại không phải cao môn. Chỉ là một kẻ đứng đầu Lục Phiến Môn. Hắn vốn định cứ thế ôm cái bát sắt từ thời tổ tiên truyền xuống, không ngờ gặp phải một tên lừa đảo dẫn tới một nơi khác. Chuyến đi này, một đi khó có ngày về.',
      'Hắn tự nhận nghĩa bạc vân thiên, thái độ làm người tứ hải, là một bằng hữu có thể gửi gắm cả thê tử cho. Nhưng chỗ tới, tất cả cao phụ quý nữ đều trốn trong khuê phòng không gặp người. Hắn tự xưng tính cách thuần lương, dám giúp mọi người làm điều tốt...',
      '***',
      'So với truyện lịch sử khác, điển hình như Cẩm Y Vệ thì Dạ Thiên Tử mang nội dung sâu sắc, có chiều sâu, mang hơi hướm quân sự. Mời bạn đọc theo dõi những diễn biến tiếp theo của câu chuyện và hứa hẹn sẽ mang đến cái kết đầy thuyết phục.',
      ''
    ],
    view: '786,689',
    date: '09:35:10 02/10/2019',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - . Bóng Dừa Dưới Trăng',
      'Chương 2 - Con Vàng',
      'Chương 3 - Giấc Ngủ Trưa',
      'Chương 4 - Kẻ Vô Hình',
      'Chương 5 - Cái Võng',
      'Chương 6 - Ai Đưa Võng Đong Đưa',
      'Chương 7 - Cái Mũ Cối',
      'Chương 8 - Bà Hàng Xóm',
      'Chương 9 - Trắng Và Đen',
      'Chương 10 - Ánh Đèn Chớp Tắt',
      'Chương 11 - Mèo Đen',
      'Chương 12 - Truyền Nhân',
      'Chương 13 - Hứa Gia',
      'Chương 14 - Ánh Lửa Trong Đêm',
      'Chương 15 - Trứng',
      'Chương 16 - Cuộc Gặp Gỡ',
      'Chương 17 - Vắng Nhà',
      'Chương 18 - Ông Ba Bị',
      'Chương 19 - Ảo Giác',
      'Chương 20 - Ma Giấu',
      'Chương 21 - Đụng Độ Với Yêu Ma',
      'Chương 22 - Cuộc Chiến Âm Dương',
      'Chương 23 - Bình Minh Mới',
      'Chương 24 - Bàn Tính',
      'Chương 25 - Đi Đám Ma',
      'Chương 26 - Căn Nhà Gỗ',
      'Chương 27 - Truy Tìm Nguồn Gốc',
      'Chương 28 - Âm Dương Hỏa Khuyển',
      'Chương 29 - Quyết Định',
      'Chương 30 - Bày Trận'
    ]
  },
  {
    id: 479,
    bookName: 'BẮC TỐNG PHONG LƯU',
    bookCover: 'https://img.dtruyen.com/public/images/large/bactongphongluuf1YpHTeeA2.jpg',
    rating: '6.2',
    language: 'Việt Nam',
    pageNo: 729,
    author: 'Nam Hi',
    genre: 'lịch sử',
    content: '',
    readed: 52,
    description: [
      'Truyện Bắc Tống Phong Lưu của tác giả Nam Hi là một truyện mới thuộc thể loại lịch sử, đô thị được gửi đến bạn đọc trên trang đọc truyện online. Cùng với những truyện nổi tiếng cùng thể loại như: Tay Súng Bắn Tỉa Lạc Về Thời Tam Quốc, Quốc Sắc Sinh Kiêu,... thì truyện cũng đã tạo cho bạn đọc một nhìn mới mẻ về lịch sử, về những gì được lưu truyền lại. Đọc truyện để tìm hiểu rõ hơn về lịch sử, đọc để có những suy niệm riêng cho chính bản thân.',
      'Lý Kỳ vốn là đầu bếp đứng đầu của một khách sạn năm sao nổi tiếng thế giới. Bởi vì uống rượu say, lại như kỳ tích xuyên việt tới Bắc Tống những năm cuối. Đi tới một thế giới xa lạ, vô thân vô cố; rơi vào đường cùng, Lý Kỳ đành phải làm lại từ đầu, với một quán ăn sắp phải mang ra bán.',
      'Lý Kỳ vốn chỉ muốn làm một tiểu đầu bếp an phận. Nhưng vàng tới nơi nào cũng sáng. Nam nhân quá xuất sắc, đôi khi không phải là một chuyện tốt.',
      '- Cao thái úy muốn ăn bò bít tết? Không thành vấn đề.',
      '- Thái thái sư muốn ăn lẩu? Bảo ông ta hẹn trước một tháng đi.',
      '- Lý Sư Sư muốn ăn salad trái cây? Ách...Ban ngày bận rồi, buổi tối hẵng tính.',
      '- Lý Thanh Chiếu muốn ăn bánh gato? Hỏi nàng ấy đã ly hôn với Triệu Minh Thành chưa?',
      '- Hoàng thượng muốn thưởng thức Kim Hán Toàn Tịch? Còn đang trong quá trình chuẩn bị, đợi mười năm sau hẵng nói.',
      'Nếu bạn đã từng thích nhân vật L3 trong “Cực phẩm gia đinh”, thì không thể bỏ qua nhân vật Lý Kỳ trong “Bắc Tống phong lưu”. Hài hước, ranh mãnh, dí dỏm, những màn đấu trí gay cấy, những diễn biến hồi hộp cùng những màn cua gái kinh điển, tạo nên một bộ truyện xuyên việt cực kỳ hấp dẫn. Hãy xem chàng Lý Kỳ của chúng ta dùng tài nấu ăn khiến cho đám gian thần, những công tử ăn chơi trác tác, những tài tử mắt cao hơn đỉnh, những mỹ nữ tuyệt sắc phải cúi đầu xưng thần như thế nào. Hãy đón đọc truyện lịch sử, quân sự đặc sắc này'
    ],
    view: '16,981,503',
    date: '20:18:07 13/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',   'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',   'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',   'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',   'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',   'Chương 10 - Chương 10',
      'Chương 11 - Chương 11', 'Chương 12 - Chương 12',
      'Chương 13 - Chương 13', 'Chương 14 - Chương 14',
      'Chương 15 - Chương 15', 'Chương 16 - Chương 16',
      'Chương 17 - Chương 17', 'Chương 18 - Chương 18',
      'Chương 19 - Chương 19', 'Chương 20 - Chương 20'
    ]
  },
  {
    id: 480,
    bookName: 'CẨM Y VỆ',
    bookCover: 'https://img.dtruyen.com/public/images/large/camyveWWxUUWSelQ.jpg',
    rating: '8.2',
    language: 'Việt Nam',
    pageNo: 1183,
    author: 'Miêu Khiêu',
    genre: 'lịch sử',
    content: '',
    readed: 2,
    description: [
      'Những cuộc phiêu lưu, những tình tiết truyện hấp dẫn, những thứ đan xen giữa thế giới hiện đại, cổ xưa trong truyện Cẩm Y Vệ làm bạn đọc sẽ không khỏi không hứng thú. Đây là một truyện được tác giả xây dựng tổng hợp nhiều yếu tố, nhờ vào trí tưởng tượng, vốn hiểu biết của mình, Miêu Khiêu đã hình thành một truyện lịch sử, quân sự độc đáo. Không còn là một truyện cứng nhắc, khô khan, với những thời gian địa điểm có sẵn nhưng đó là sự linh hoạt, sống động lồng ghép cả tính chất truyện kiếm hiệp vào tác phẩm.',
      'Truyện xoay quanh Tần Lâm đến từ thế kỷ hai mươi mốt, là một cao thủ trinh sát hình sự nhiều lần phá đại án, kỹ thuật pháp y tinh xảo, bốn năm học đại học Công An Nhân Dân Hoa Hạ hệ chính quy khoa điều tra kỹ thuật hình sự. Sau công tác tại học viện cảnh sát hình sự Hoa Hạ tiếp tục học lấy được bằng Thạc sĩ pháp y, trong lúc nhậm chức nhiều lần phá được án lớn, quan trọng do bộ Công An đốc thúc, chỉ mấy năm đã thăng tiến thành Cảnh Đốc cấp hai.',
      'Không ngờ trong một lần thi hành nhiệm vụ phát sinh tai nạn xe cộ, cả người lẫn xe rơi xuống vực sâu trăm trượng. Đến khi tỉnh lại phát hiện ra toàn thân mình lõa thể nằm ngoài vùng hoang dã, thân thể đã hóa thành bộ dáng thiếu niên chỉ mới mười lăm mười sáu tuổi.',
      'Sau mới phát hiện thì ra mình đã trở lại triều Minh, Tần Lâm sẽ làm thế nào để tồn tại giữa một thế giới vô cùng xa lạ; có thể hay không tồn tại và đứng vững, có hay không sẽ bị lung lạc... Mời bạn đón đọc truyện.',
      ''
    ],
    view: '6,046,808',
    date: '13:14:56 10/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Hai Người Chồng Ma',
      'Chương 2 - Kết Hôn Với Anh Ta Thì Phải Chết',
      'Chương 3 - Trả Hết Thù Mới Với Nợ Cũ',
      'Chương 4 - Bên Trong Lò Thiêu',
      'Chương 5 - Giấc Mơ Tối Hôm Qua Là Thật?',
      'Chương 6 - Nuôi Quỷ Nuôi Thi',
      'Chương 7 - Sống Phải Thấy Người Chết Phải Thấy Xác',
      'Chương 8 - Một Vợ Nhiều Chồng',
      'Chương 9 - Đạo Bào Của Bà Ngoại',
      'Chương 10 - Hương Hương Cứu Mạng',
      'Chương 11 - Xử Lý Thích Uý Nhiên',
      'Chương 12 - Thay Trời Hành Đạo',
      'Chương 13 - Chồng Ma Mau Cứu Em',
      'Chương 14 - Lưng Của Tôi Đau Quá',
      'Chương 15 - Chồng Ma Đừng Lấn Tới',
      'Chương 16 - Cậu Đừng Chạy Lung Tung',
      'Chương 17 - Con Gái Ma',
      'Chương 18 - Tuyết Nhi',
      'Chương 19 - Chạy Thoát Quan Trọng Hơn',
      'Chương 20 - Tôi Là Chấp Niệm Cuối Cùng Của Anh',
      'Chương 21 - Đi Đêm Lắm Có Ngày Gặp Ma',
      'Chương 22 - Dấu Ấn Của Ác Quỷ',
      'Chương 23 - Đốt Quần Áo Cho Tôi',
      'Chương 24 - Trang Điểm Cho Xác Chết',
      'Chương 25 - Chồng Ma Rất Mạnh Mẽ',
      'Chương 26 - Quỷ Chị Còn Không Sợ',
      'Chương 27 - Luyện Cương Thi Kinh Khủng',
      'Chương 28 - Cương Thi Chú Đặng',
      'Chương 29 - Không Chống Nổi',
      'Chương 30 - Lời Hứa Hôn Trước Khi Chết'
    ]
  },
  {
    id: 481,
    bookName: 'THIÊN HẠ',
    bookCover: 'https://img.dtruyen.com/public/images/large/thienhatAFx1um417.jpg',
    rating: '6.2',
    language: 'Việt Nam',
    pageNo: 784,
    author: 'Cao Nguyệt',
    genre: 'lịch sử',
    content: '',
    readed: 99,
    description: [
      'Bạn đang đọc truyện Thiên Hạ của tác giả Cao Nguyệt trên trang đọc truyện online.Năm Thiên Bảo thứ năm, Đại Đường lập nước đã qua trăm năm, trải qua gần mười năm trị quốc, Lý Long Cơ đã biết thiên mệnh hơi mỏi mệt, từ sau khi hắn sắc phong Dương thị làm Quý phi, tâm tư của hắn cũng dần dần rời bỏ triều chính buồn tẻ mà lộn xộn.“Suốt ngày múa hát thung dung, tiếng tơ tiếng trúc sau lòng quân vương”.',
      '(Trích Trường Hận Ca – Bạch Cư Dị - Tản Đà)Hắn bắt đầu thỏa thích trong âm nhạc ca múa, từ nay về sau Hoàng đế không triều sớm…Mà ngay mùa xuân năm này, một nhân vật mới đã đến Túc Lâu Phong Thú Bảo An Tây, một kẻ xuyên qua đến từ đời sau, trước vào quân đội trấn thủ biên cương, chống địch bên ngoài mới hiện tài hoa, lại đưa Công chúa vào kinh, bước vào trung tâm quyền lực của Đại Đường.',
      'Trong thời gian ngắn ngủn, hắn từ một nam nhi kiên cường đời sau, biến thành một vị anh hùng thịnh thế trí dũng song toàn. Đây là một bộ đại tác “Đường xuyên” (tác phẩm lớn xuyên qua Đại Đường) độ dài rất lớn, bắt đầu bởi tác giả Cao Nguyệt nổi tiếng hành văn cao siêu, vì bạn mà bày ra vở kịch lớn xuyên việt Thịnh Đường hết sức đặc sắc, chiến tranh tái ngoại (phía bắc Trường thành) nổ ra, cảnh tàn sát khốc liệt trên triều đường, ngồi xem giang sơn như bức họa, lại có mỹ nhân như ngọc, trong nhất thời giang sơn mỹ nhân, hết thảy Thịnh Đường uốn lượn quanh co'
    ],
    view: '4,433,894',
    date: '00:25:00 03/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Q.1 - Chương 1 - Chương 1',
      'Q.1 - Chương 2 - Chương 2',
      'Q.1 - Chương 3 - Chương 3',
      'Q.1 - Chương 4 - Chương 4',
      'Q.1 - Chương 5 - Chương 5',
      'Q.1 - Chương 6 - Chương 6',
      'Q.1 - Chương 7 - Chương 7',
      'Q.1 - Chương 8 - Chương 8',
      'Q.1 - Chương 9 - Chương 9',
      'Q.1 - Chương 10 - Chương 10',
      'Q.1 - Chương 11 - Chương 11',
      'Q.1 - Chương 12 - Chương 12',
      'Q.1 - Chương 13 - Chương 13',
      'Q.1 - Chương 14 - Chương 14',
      'Q.1 - Chương 15 - Chương 15',
      'Q.1 - Chương 16 - Chương 16',
      'Q.1 - Chương 17 - Chương 17',
      'Q.1 - Chương 18 - Chương 18',
      'Q.1 - Chương 19 - Chương 19',
      'Q.1 - Chương 20 - Chương 20',
      'Q.1 - Chương 21 - Chương 21',
      'Q.1 - Chương 22 - Chương 22',
      'Q.2 - Chương 1 - Chương 1',
      'Q.2 - Chương 2 - Chương 2',
      'Q.2 - Chương 3 - Chương 3',
      'Q.2 - Chương 4 - Chương 4',
      'Q.2 - Chương 5 - Chương 5',
      'Q.2 - Chương 6 - Chương 6',
      'Q.2 - Chương 7 - Chương 7',
      'Q.2 - Chương 8 - Chương 8'
    ]
  },
  {
    id: 482,
    bookName: 'TRÍ TUỆ ĐẠI TỐNG',
    bookCover: 'https://img.dtruyen.com/public/images/large/trituedaitongf3XwHlXKW8.jpg',
    rating: '6.5',
    language: 'Việt Nam',
    pageNo: 635,
    author: 'Kiết Dữ 2',
    genre: 'lịch sử',
    content: '',
    readed: 85,
    description: [
      'Ai là fan của Đường Chuyên thì bơi hết vào đây cho ad Phan của đường chuyên thì không thể bỏ qua truyện này nha. Tuy không thể vượt qua cái bóng quá lớn của Đường Chuyên nhưng bộ này vẫn top 5 những truyện lịch sử quân sự đọc nhiều nhất trước nay.',
      'Ngày 9 tháng 1 năm 2014 trong khi vẫn tiếp tục mỗi ngày đăng Đường Chuyên, tác phẩm lịch sử giúp mình nhất bộ phong thần, Kiết Dữ 2 gây chấn động khi tung ra tác phẩm tiếp theo Trí Tuệ Đại Tống, trở thành một trong số rất ít tác giả có thể cùng lúc cho đăng hai tác phẩm mà vẫn giữ được chất lượng.',
      'Công bằng mà nói ở những chương đầu vì muốn thoát khỏi cái bóng lớn của Đường Chuyên, tác giả viết có phần viết hơi lộn xộn mất phương hướng, vẫn bị ảnh hưởng bởi Đường Chuyên, rõ ràng nhất là liên tục nhầm tên nhân vật chính của hai truyện, nhưng phải nói cuối cùng tác giả thành công khi thể hiện được sự khác biệt của Trí Tuệ Đại Tống với tác phẩm thành danh của mình.',
      'Từ khi truyện phát hành liên tục thống trị bxh thể loại lịch sử quân sự, với nửa năm đã leo lên top 5 những truyện lịch sử quân sự đọc nhiều nhất trước nay, xếp ngay sau Đường Chuyên.',
      ''
    ],
    view: '1,945,419',
    date: '14:46:22 10/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',   'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',   'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',   'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',   'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',   'Chương 10 - Chương 10',
      'Chương 11 - Chương 11', 'Chương 12 - Chương 12',
      'Chương 13 - Chương 13', 'Chương 14 - Chương 14',
      'Chương 15 - Chương 15', 'Chương 16 - Chương 16',
      'Chương 17 - Chương 17', 'Chương 18 - Chương 18',
      'Chương 19 - Chương 19', 'Chương 20 - Chương 20',
      'Chương 21 - Chương 21', 'Chương 22 - Chương 22',
      'Chương 23 - Chương 23', 'Chương 24 - Chương 24',
      'Chương 25 - Chương 25', 'Chương 26 - Chương 26',
      'Chương 27 - Chương 27', 'Chương 28 - Chương 28',
      'Chương 29 - Chương 29', 'Chương 30 - Chương 30'
    ]
  },
  {
    id: 483,
    bookName: 'ĐẠI ĐƯỜNG TIỂU LANG TRUNG',
    bookCover: 'https://img.dtruyen.com/public/images/large/daiduongtieulangtrung4l2EDH3Z73.jpg',
    rating: '6.7',
    language: 'Việt Nam',
    pageNo: 668,
    author: 'Mộc Dật',
    genre: 'lịch sử',
    content: '',
    readed: 85,
    description: [
      'Truyện Đại Đường Tiểu Lang Trung với thể loại kiếm hiệp và các nguyên tắc trước đây dường như đã được phá vỡ vì đây là một quy tắc mới do Mộc Dật đề cập đến. Bởi tác giả luôn tạo nên tên tuổi của mình cùng phong cách truyện riêng biệt.',
      'Bắt đầu từ những ngày Đại Đương lập quốc, trong huyện thành Thạch Kính có một hiệu thuốc tên Quý Chi Đường, chủ hiệu thuốc Tả Quý y thuật bình thường, thê tử Lương thị nhu nhược nhút nhát, nhi nữ đanh đá thương đệ đệ, cả nhà sinh hoạt hết sức gian nan, hiệu thuốc ít khách đổ bể tới nơi.',
      'Và những ngày chuẩn bị bước sang năm mới rồi trong nhà chỉ có banh bao đen trộn rau dại, còn bị chủ nhà đòi nợ, không nộp được tiền sẽ đuổi khỏi nhà ...Tả Thiếu Dương là nhi tử độc nhất của nhà đó, từ đây y bước trên con đường hành y đầy chông gai, cũng không thiếu bóng hồng bầu bạn. Truyện đã được hoàn thành mới các quý độc giả theo dõi nhé.',
      ''
    ],
    view: '815,548',
    date: '20:08:32 05/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Bắt Đầu Câu Chuyện',
      'Chương 2 - Nice To Meet You, Nguyen!',
      'Chương 3 - You Are Invited',
      'Chương 4 - Before The Story Begin, You Have To Know The Author',
      'Chương 5 - The Dress! Do You Remember Who You Are?',
      'Chương 6 - Party Of The Criminal',
      "Chương 7 - Devil's Love",
      'Chương 8 - Chương 8',
      'Chương 9 - Another Story',
      'Chương 10 - Chương 10',
      "Chương 11 - Devil & Vampire, What's The Different?",
      'Chương 12 - Chương 12',
      'Chương 13 - Chương 13',
      'Chương 14 - Chương 14',
      'Chương 15 - His Feeling',
      'Chương 16 - Chương 16',
      'Chương 17 - Chương 17',
      'Chương 18 - Chương 18',
      'Chương 19 - Chương 19',
      'Chương 20 - Mad Scientist',
      'Chương 21 - Chương 21',
      'Chương 22 - Murder House (1)',
      'Chương 23 - Murder House (2)',
      'Chương 24 - Murder House (3)',
      'Chương 25 - Murder House (4)',
      'Chương 26 - Chương 26',
      'Chương 27 - I’M Jonathan Henrold, Not Mars.',
      'Chương 28 - Chương 28',
      'Chương 29 - Mảnh Gương Vỡ',
      'Chương 30 - Ai Cũng Phải Yêu Em'
    ]
  },
  {
    id: 484,
    bookName: 'SAY MỘNG GIANG SƠN',
    bookCover: 'https://img.dtruyen.com/public/images/large/saymonggiangsonbaReIxhG6G.jpg',
    rating: '7',
    language: 'Việt Nam',
    pageNo: 1221,
    author: 'Nguyệt Quan',
    genre: 'lịch sử',
    content: '',
    readed: 11,
    description: [
      'Bạn đang đọc truyện Say Mộng Giang Sơn của tác giả Nguyệt Quan trên trang đọc truyện online.Truyện xảy ra vào những năm vàng son của nữ hoàng đế Võ Chiếu (Võ Tắc Thiên);',
      'Thượng Quan Uyển Nhi danh chấn thiên hạ; Lý Khỏa Nhi nổi tiếng với sắc đẹp nguyệt thẹn hoa nhường; con cháu năm họ nổi tiếng lớn nhất thời đó (họ Thôi ở huyện An Bình, và huyện Thanh Hà vùng Hà Bắc; họ Lô ở Phạm Dương; họ Lý ở Lũng Tây và ở huyện Triệu vùng Hà Bắc; họ Trịnh ở Huỳnh Dương; họ Vương ở Thái Nguyên) lục đục, cấu xé lẫn nhau.',
      'Thái Bình công chúa thì chẳng có được thái bình; nhà họ Lý ba đời làm quan nền tảng thật vững chắc; Địch Nhân Kiệt, Trương Dịch Chi, Phùng Tiểu Bảo, nào tài tử, nào giai nhân, lớp lớp hào kiệt chi sĩ! Anh hùng hiệp khách, bóng hồng gọi mời, cho dù có rong chơi phiêu lãng khắp Đại Đường rồi cuối cùng cũng say đắm trong sự dịu dàng, thùy mị của mỹ nhân mà dừng bước chân giang hồ!'
    ],
    view: '2,008,983',
    date: '14:43:25 10/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 01 - Phần 1',
      'Chương 2 - Chương 01 - Phần 2',
      'Chương 3 - Chương 02 - Phần 1',
      'Chương 4 - Chương 02 - Phần 2',
      'Chương 5 - Chương 03',
      'Chương 6 - Chương 04 - Phần 1',
      'Chương 7 - Chương 04 - Phần 2',
      'Chương 8 - Chương 05 - Phần 1',
      'Chương 9 - Chương 05 - Phần 2',
      'Chương 10 - Chương 06',
      'Chương 11 - Chương 07 - Phần 1',
      'Chương 12 - Chương 07 - Phần 2',
      'Chương 13 - Chương 08 - Phần 1',
      'Chương 14 - Chương 08 - Phần 2',
      'Chương 15 - Chương 09 - Phần 1',
      'Chương 16 - Chương 09 - Phần 2',
      'Chương 17 - Chương 10 - Phần 1',
      'Chương 18 - Chương 10 - Phần 2',
      'Chương 19 - Chương 11 - Phần',
      'Chương 20 - Chương 11 - Phần 1',
      'Chương 21 - Chương 12',
      'Chương 22 - Chương 13 - Phần 1',
      'Chương 23 - Chương 13 - Phần 2',
      'Chương 24 - Chương 14',
      'Chương 25 - Chương 15 - Phần 1',
      'Chương 26 - Chương 15 - Phần 2',
      'Chương 27 - Chương 16 - Phần 1',
      'Chương 28 - Chương 16 - Phần 2',
      'Chương 29 - Chương 17 - Phần 1',
      'Chương 30 - Chương 17 - Phần 2'
    ]
  },
  {
    id: 485,
    bookName: 'ĐƯỜNG CHUYÊN',
    bookCover: 'https://img.dtruyen.com/public/images/large/duongchuyens2bSy8Akx7.jpg',
    rating: '7.9',
    language: 'Việt Nam',
    pageNo: 1499,
    author: 'Kiết Dữ 2',
    genre: 'lịch sử',
    content: '',
    readed: 62,
    description: [
      'Bạn đang đọc truyện Đường Chuyên của tác giả Kiết Dữ 2. Nhiều người vẫn hay lầm tưởng thể loại truyện lịch sử, quân sự rất khô khan, khó đọc, truyện cũng không có gì thu hút, chỉ là những truyện lịch sử được truyền đi truyền lại theo những bài học hàng ngày. Thực ra không phải đại đa số đều vậy,có rất nhiều truyện thu hút, lý thú và vui nhộn, truyện Đường Chuyên là một trong những truyện ấy.',
      'Truyện được tác giả Kiết Dữ xây dựng thực sống động và đầy hóm hỉnh, chính lối viết này đã làm cho Đường Chuyên trở thành một bộ truyện độc đáo, Kiết Dữ chen chân vào hàng top những tác giả được yêu thích nhất',
      'Đường Chuyên với phong cách nhẹ nhàng, hóm hỉnh nhưng có nền tảng lịch sử sâu dầy đã tạo nên sự đột phá điên đảo thể loại lịch sử quân sự. Lịch sử không thể lặp lại, nhưng lịch sử có thể viết lại, còn ai có thể viết lịch sử thoải mái mà không tùy tiện như Kiết Dữ 2?',
      'Vân Diệp chỉ là người bình thường, nhưng người bình thường như vô vạn người bình thường ấy laaijc ó cơ hội xuyên việt, y tới những năm đầu Trinh Quan thời Đường, đi cùng y chỉ có một con ngựa hoang gặp nạn, y không hiểu quân sự, không hiểu chính trị, y chẳng thể lật tay làm mây úp tay làm mưa, nhưng y định sẵn sẽ làm thay đổi thời đại do kiếm và bút tô vẽ này. Sơ Đường có quá nhiều tiếc nuối, Trường Tôn hoàng hậu chết sớm, công chúa sống xoa hoa dâm dật, huynh đệ tương tàn, Vân Diệp nguyện làm một viên gạch lát nền cho Đại Đường để bù đắp nuối tiếc đó, để Đại Đường thêm tình người, chứ không phải chỉ có quyền lợi chinh phạt lạnh băng. Con đường đã chọn này nhiều chông gai hay đã có ai trải bước, cùng đọc truyện để đồng hành với Vân Diệp trên con đường viết lại lịch sử này. Nếu bạn yêu thích tác giả Kiết Dữ 2 thì có thể đọc thêm: Ngân Hồ, Trí Tuệ Đại Tống',
      ''
    ],
    view: '5,200,963',
    date: '20:02:49 05/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',
      'Chương 2 - Chương 1.2',
      'Chương 3 - Chương 2',
      'Chương 4 - Chương 2.2',
      'Chương 5 - Chương 3',
      'Chương 6 - Chương 3.2',
      'Chương 7 - Chương 4',
      'Chương 8 - Chương 4.2',
      'Chương 9 - Chương 5',
      'Chương 10 - Chương 5.2',
      'Chương 11 - Chương 6',
      'Chương 12 - Chương 6.2',
      'Chương 13 - Chương 7',
      'Chương 14 - Chương 7.2',
      'Chương 15 - Chương 8',
      'Chương 16 - Chương 8.2',
      'Chương 17 - Chương 9',
      'Chương 18 - Chương 9.2',
      'Chương 19 - Chương 10',
      'Chương 20 - Chương 10.2',
      'Chương 21 - Chương 11',
      'Chương 22 - Chương 11.2',
      'Chương 23 - Chương 12',
      'Chương 24 - Chương 12-2',
      'Chương 25 - Chương 13',
      'Chương 26 - Chương 13.2',
      'Chương 27 - Đại Không Vong*',
      'Chương 28 - Đại Không Vong* 2',
      'Chương 29 - Chương 15',
      'Chương 30 - Chương 15-2'
    ]
  },
  {
    id: 494,
    bookName: 'VƯƠNG GIA ĐẠI THÚC NGƯỜI THẬT XẤU',
    bookCover: 'https://img.dtruyen.com/public/images/large/vuonggiadaithucnguoithatxauri7fkK61Fe.jpg',
    rating: '8.3',
    language: 'Việt Nam',
    pageNo: 1053,
    author: 'Ninh Khuynh',
    genre: 'lịch sử',
    content: '',
    readed: 96,
    description: [
      'Mang nội dung độc đáo và thú vị, Vương Gia Đại Thúc Người Thật Xấu là truyện xuyên không mới lạ so với Tướng Công Bám Người vàVương Phi Mười Ba Tuổi. Truyện kể về nàng Hạ Thiên, người con gái ở thế kỉ 21 mở tiệm thuốc A, là một người phụ nữ của thời đại mới.',
      'Trong một lần xuyên không lại bị cường nhân cưỡng hôn ôm ấp. Ai có thể ngờ được một đại thúc miệng đầy ria mép, bề ngoài tầm thường thế kia lại có thể là một vị Vương gia được thiên hạ xưng tụng là đệ nhất mỹ nam?',
      'Nàng quẫn! Không muốn bị đem đi thành thân thay cho người khác, nàng đành phải trốn thoát! “Đại gia, người có muốn mua một hộp về dùng thử hay không?” Một bé trai đáng yêu giơ hộp thuốc ra trước mặt hắn, khuôn mặt sáng ngời. Khóe miệng của vị Vương gia đại thúc co giật: “Con gọi ta là gì?”',
      '“Đại gia nha, mẹ của con gọi người là đại thúc, con chắc chắn gọi người là đại gia rồi, tới mua một hộp đi đại gia, con sẽ giảm giá cho người”.',
      'Hắn giận dữ: “Ta là cha con!”',
      'Truyện cho bạn đọc nhiều tình huống mở đầy bất ngờ và thú vị khi thể loại xuyên không đã mang cốt truyện khá mờ ảo, truyện giúp người đọc như tìm được lối ra nhưng cứ lẫn quẩn không thể ra. Hơn thế nữa, bạn đọc có thể đến với đọc truyện online để theo dõi tiếp diễn biến câu chuyện và tìm kiếm những tựa truyện hấp dẫn khác.',
      ''
    ],
    view: '1,113,085',
    date: '20:16:22 06/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',
      'Chương 2 - Cái Chết Kỳ Quặc',
      'Chương 3 - Nửa Đêm Ra Sau Núi',
      'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',
      'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',
      'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',
      'Chương 10 - Chương 10',
      'Chương 11 - Chương 11',
      'Chương 12 - Chương 12',
      'Chương 13 - Chương 13',
      'Chương 14 - Chương 14',
      'Chương 15 - Chương 15',
      'Chương 16 - Chương 16',
      'Chương 17 - Chương 17',
      'Chương 18 - Chương 18',
      'Chương 19 - Chương 19',
      'Chương 20 - Chương 20',
      'Chương 21 - Chương 21',
      'Chương 22 - Chương 22',
      'Chương 23 - Chương 23',
      'Chương 24 - Chương 24',
      'Chương 25 - Chương 25',
      'Chương 26 - Chương 26',
      'Chương 27 - Chương 27',
      'Chương 28 - Chương 28',
      'Chương 29 - Chương 29',
      'Chương 30 - Chương 30'
    ]
  },
  {
    id: 503,
    bookName: 'TRỰC TUYẾN CÔNG CUỘC KÉO DÀI MẠNG SỐNG CỦA NỮ CHÍNH',
    bookCover: 'https://img.dtruyen.com/public/images/large/tructuyencongcuockeodaimangsongcuanuchinhcTEw6988Gc.jpg',
    rating: '8.5',
    language: 'Việt Nam',
    pageNo: 1251,
    author: 'Chung Dục',
    genre: 'lịch sử',
    content: '',
    readed: 11,
    description: [
      'Bạn đang đọc truyện Trực Tuyến Công Cuộc Kéo Dài Mạng Sống Của Nữ Chính của tác giả Chung Dục. Khi bạn không cẩn thận chết đi thì phải làm sao đây? Bó tay! Nhưng mà, nếu bạn có một hệ thống có thể giúp bạn sống lại lần nữa thì sao nhỉ? Thế thì bất kể làm gì chăng nữa đều phải kéo dài cái mạng này ra.',
      'Tô Ngọc Tuyết: Được, chị làm được, chị tuyệt đối sẽ không để mình chết đi!',
      'Hệ thống: Phải gắng gượng!',
      'Tô Ngọc Tuyết:...Mệt quá à (nằm dài)',
      'Hệ thống: Nhìn túi quà tặng của tôi này! Cô cầm lấy mà gắng gượng nào, cố lên!',
      'Tô Ngọc Tuyết: Ôm lấy túi lì xì lớn chính là chạy nước rút một trăm mét.jpg',
      'Dương Tiễn: Hửm?',
      'Tô Ngọc Tuyết: ......(╯‵□′)╯︵┻━┻ Rút đao đi, hệ thống!',
      'Cả bộ truyện kể về những thế giới mà nữ chính xuyên qua để thực hiện nhiệm vụ, phải cố gắng kéo dài mạng sống cũng phải níu kéo cả nam thần!'
    ],
    view: '8,150',
    date: '08:08:11 14/10/2020',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - MẪU TỬ SÁT THI',
      'Chương 2 - KHAI QUAN',
      'Chương 3 - HỎA THIÊU SÁT THI',
      'Chương 4 - MIÊU CƯƠNG CỔ SƯ',
      'Chương 5 - QUAN MÔN ĐỆ TỬ',
      'Chương 6 - THÀNH TÀI TUNG HOÀNH THIÊN HẠ',
      'Chương 7 - CAO NHÂN',
      'Chương 8 - QUỶ ĐẬP TƯỜNG',
      'Chương 9 - XÉT NHÀ',
      'Chương 10 - NƯỚC ĐỌNG',
      'Chương 11 - THỦY THI',
      'Chương 12 - NỤ HÔN ĐẦU TIÊN HIẾN CHO QUỶ',
      'Chương 13 - Lamborghini',
      'Chương 14 - Giúp Mặc Quần',
      'Chương 15 - Mời Đi Vào Giấc Mộng',
      'Chương 16 - Tà Linh',
      'Chương 17 - Thi Ma',
      'Chương 18 - Oán Linh Thị Nữ',
      'Chương 19 - Cổ Mộ.',
      'Chương 20 - TỨ HUYẾT DIỆT THI HOÀN',
      'Chương 21 - Dũng Cảm Đấu Thi Ma',
      'Chương 22 - Thủ Linh Đồng Tử',
      'Chương 23 - SIÊU ĐỘ OÁN LINH',
      'Chương 24 - LẦN THỨ HAI ĐI VÀO GIẤC MỘNG',
      'Chương 25 - QUỶ KÝ GIẢ',
      'Chương 26 - LẦU BỐN KÝ TÚC XÁ',
      'Chương 27 - NẠO XƯƠNG',
      'Chương 28 - Huyết Tinh Phù.',
      'Chương 29 - Thẩm Vấn Quỷ',
      'Chương 30 - Hẹn Hò'
    ]
  },
  {
    id: 507,
    bookName: 'MẬT THÁM PHONG VÂN',
    bookCover: 'https://img.dtruyen.com/public/images/large/matthamphongvancUQnhhYjUF.jpg',
    rating: '8.3',
    language: 'Việt Nam',
    pageNo: 640,
    author: 'Thần Long',
    genre: 'lịch sử',
    content: '',
    readed: 62,
    description: [
      'Truyện Mật Thám Phong Vân kể về một thanh niên tên Hàn Phong, chết nhảm nhí, sau được Diêm Vương cho vào luân hồi không bị mất ký ức, hắn sẽ ra sao?',
      'Hắn sẽ gặp được bằng hữu, kẻ thù, mỹ nữ. Đến cuối cùng, ai sẽ còn lại bên cạnh hắn?',
      'Hắn sẽ tăng tiến bản thân, tiền tài, quyền lực, võ công. Đến cuối cùng, là vì cái gì?',
      'Càng lấn sâu vào thế giới mới, hắn sẽ dần mở khóa những câu chuyện, và giải mã một bí ẩn kinh thiên thời thái cổ.',
      'Lời tác giả:',
      'Bộ truyện này tuy là đầu tay, nhưng mình thực sự đầu tư/chuẩn bị khi viết, mong được quý vị ủng hộ. Nếu nghiện lịch sử - quân sự, vậy truyện này chính là một lựa chọn.',
      'Truyện này lấy võ hiệp làm chính. Nhiều chỗ sẽ thấy quen, ví dụ danh tự, nhưng một số quy tắc/trình tự do mình tự nghĩ ra, không biết có trùng với ai đó chưa? Tuy nhiên tất cả mảnh ghép phải đến những chương cuối mới thấy được.',
      'Tranh chấp sẽ đa dạng. Từ giang hồ, buôn bán, quân đội, triều đình, ... Mình không hy vọng truyện đơn thuần chỉ luyện, rồi đánh nhau, rồi nổi danh, giàu có, rồi mỹ nữ vây lấy, rồi vô đối, giả ngu đi khinh thị người khác ... Cuộc sống xung quanh đâu có dễ dàng như vậy.',
      'Tình tiết đôi chỗ sẽ chậm. Chậm nhưng không chán. Từng tình huống mình đều cố gắng nhìn từ nhiều góc độ, nhiều cá nhân, để chi ra rằng, cùng một sự việc, mỗi người một cách lý giải, một kiểu toan tính, cuộc sống muôn màu.',
      'Mỹ nữ? Tài nữ? Dĩ nhiên không thể thiếu, tâm tư hành động đều đi vào cụ thể, sẽ không để làm "bình hoa" nhàm chán.',
      'Chính-tà-đúng-sai, không có gì là tuyệt đối. Mỗi cá nhân là một thế giới, họ cho là đúng, người khác cho là sai. Ai mới là người phân định?',
      'Đọc truyện đôi khi quý huynh đệ sẽ không thích nhân vật chính. Không sao? Chính là vậy, hắn không hề hoàn hảo, có thể không hợp ý là điều dễ hiểu. Xung quanh hắn ta còn có rất nhiều người, tốt hơn, giỏi hơn, thành công hơn, ... nhưng điều quan trọng, hắn ta là "đồng hương" của chúng ta, cùng nhau xuất phát và phiêu lưu trong thế giới này.'
    ],
    view: '772,908',
    date: '12:47:27 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',   'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',   'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',   'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',   'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',   'Chương 10 - Chương 10',
      'Chương 11 - Chương 11', 'Chương 12 - Chương 12',
      'Chương 13 - Chương 13', 'Chương 14 - Chương 14',
      'Chương 15 - Chương 15', 'Chương 16 - Chương 16',
      'Chương 17 - Chương 17', 'Chương 18 - Chương 18',
      'Chương 19 - Chương 19', 'Chương 20 - Chương 20',
      'Chương 21 - Chương 21', 'Chương 22 - Chương 22',
      'Chương 23 - Chương 23', 'Chương 24 - Chương 24',
      'Chương 25 - Chương 25', 'Chương 26 - Chương 26',
      'Chương 27 - Chương 27', 'Chương 28 - Chương 28',
      'Chương 29 - Chương 29', 'Chương 30 - Chương 30'
    ]
  },
  {
    id: 508,
    bookName: 'NHẤT THỐNG THIÊN HẠ',
    bookCover: 'https://img.dtruyen.com/public/images/large/nhatthongthienhaJ9L9SXYnZX.jpg',
    rating: '9.1',
    language: 'Việt Nam',
    pageNo: 597,
    author: 'sheepboy',
    genre: 'lịch sử',
    content: '',
    readed: 43,
    description: [
      'Truyện Nhất Thống Thiên Hạ kể về Lý Hạo, người từng là một giai thoại, một cái gì đó to lớn với trẻ em trong xóm. Phía sau dáng vẻ hiền lành, nụ cười hòa ái đó, lại là một con người hoàn toàn khác. Lý Hạo còn có một thân phận mà không ai trong khu biệt thự Lan Tím ngờ được. Hắn chính là ông trùm của thế giới ngầm ở Sài Gòn. Có thể nói, trên đất Sài Gòn này, Lý Hạo lật tay làm mây, ngửa tay làm mưa cũng không ngoa. Một tay của hắn có thể che trời, một cú dậm chân của hắn có thể làm rung chuyển cả Sài Gòn.',
      'Một ông trùm xã hội đen khi về làm vua thời cổ đại sẽ gặp những cảnh ngộ ra sao? Một đất nước loạn lạc khắp nơi có được bình định hay không?',
      'Một đế chế hùng mạnh sẽ được dựng nên như thế nào? Một hôn quân, một bạo chúa, hay một minh quân sẽ xuất thế?',
      'Truyện là một truyện xuyên không khá hấp dẫn lấy bối cảnh hết sự thực tế, gần gũi, lối kể chuyện của tác giả lại sinh động, thu hút bạn đọc. Đọc truyện bạn có thể trải nghiệm một chút cảm giác phiêu lưu, một chút suy nghĩ, một chút chiêm nghiệm, hãy đọc và cảm nhận'
    ],
    view: '175,083',
    date: '00:00:00 03/01/2014',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Cô Là Nô Lệ Của Ta',
      'Chương 2 - Hít Thở Không Thông',
      'Chương 3 - Cô Càng Thống Khổ, Ta Càng Vui',
      'Chương 4 - Điên Rồi!',
      'Chương 5 - Sống Không Bằng Chết',
      'Chương 6 - Liên Luỵ',
      'Chương 7 - Sợ Hãi',
      'Chương 8 - Sinh Con Cho Bổn Toạ',
      'Chương 9 - Diệt Tộc',
      'Chương 10 - Tra Tấn',
      'Chương 11 - Bất Lực',
      'Chương 12 - Dục Vọng',
      'Chương 13 - Hầm Mộ Cổ',
      'Chương 14 - Thăm Dò',
      'Chương 15 - Mặc Ca Ca',
      'Chương 16 - Dâm Loạn',
      'Chương 17 - Chứng Cứ',
      'Chương 18 - Tôi Tin Anh',
      'Chương 19 - Một Chữ Cũng Không Tin',
      'Chương 20 - Cứu Nam Nhân',
      'Chương 21 - Tịch Liễu Căng Thẳng',
      'Chương 22 - Huynh Yêu Ta Đến Nhường Nào',
      'Chương 23 - Cô Phải Chết',
      'Chương 24 - Biến Mất',
      'Chương 25 - Người Nam Nhân Kia',
      'Chương 26 - Thả Tự Do Cho Nàng',
      'Chương 27 - Người Tôi Yêu Vẫn Luôn Là Anh',
      'Chương 28 - Không Tiếc Bất Cứ Giá Nào',
      'Chương 29 - Đã Quên',
      'Chương 30 - Quấy Nhiễu Đêm Động Phòng'
    ]
  },
  {
    id: 509,
    bookName: 'HÌNH ĐỒ',
    bookCover: 'https://img.dtruyen.com/public/images/large/hinhdorfxC1eAbcg.jpg',
    rating: '6.7',
    language: 'Việt Nam',
    pageNo: 793,
    author: 'Canh Tân',
    genre: 'lịch sử',
    content: '',
    readed: 27,
    description: [
      'Đọc truyện Hình Đồ của tác giả Canh Tân chương mới nhất full prc pdf ebook hỗ trợ xem trên thiết bị di động hoặc xem trực tuyến tại dtruyen.com',
      'Truyện Hình Đồ là một trong những truyện thuộc thể loại Lịch Sử và Quân Sự nhưng cũng pha lẫn đôi chút yếu tố huyền huyễn...nổi tiếng của tác giả Canh Tân. Trong bối cảnh câu chuyện, nhân vật chính của chúng ta là Lưu Sơn Quân, kiếp trước từng là một chuyên gia dinh dưỡng, cuộc sống luôn có kế hoạch, điều độ, nhất là trong chế độ rèn luyện thân thể, ăn uống nên cơ thể khỏe mạnh, dẻo dai, trẻ trung, tính cách thích phiêu lưu mạo hiểm, kích thích. Trong một lần vượt biển trở về quê nhà sau chuyến phiêu lưu, tàu của Lưu Sơn Quân đã gặp nạn, Lưu Sơn Quân bị chết hồn xuyên nhập vào Lưu Khám, một thiếu niên mười lăm tuổi vừa chết vì bảo vệ gia đình Lữ gia đi đến huyện Bái.',
      'Lưu Khám (tức Lưu Sơn Quân) trọng sinh vào đúng thời điểm Tần Thủy Hoàng vừa nhất thống sáu nước, lập ra nhà Tần lên ngôi hoàng đế, bắt đầu mở ra một cuộc cải cách lớn về chính trị và kinh tế.',
      'Và cuộc hội ngộ giữa Lưu Khám với những nhân vật lịch sử thời Tần Hán đã bắt đầu. Người hắn gặp đầu tiên là Lữ Trĩ, vị Lữ Hậu nổi tiếng trong lịch sử, và ngay sau đó, là cuộc chiến với đám đạo phỉ do Lưu Bang (người sáng lập nên nhà Hán) cầm đầu đánh cướp cả nhà Lữ gia, cũng từ đây đã tạo nên một cuộc hội ngộ kỳ lạ giữa Lưu Khám với Lưu Bang và cũng là khởi đầu mối hận thù giữa Lưu Khám và Lưu Bang (vì Lưu Sơn Quân ngay khi trọng sinh đã giết vợ Lưu Bang.)',
      'Những nhân vật nổi danh trong lịch sử sau đó cũng dần dần xuất hiện: Phàn Khoái, Tiêu Hà, Hạ Hầu Anh, Chu Bột, Lư Quán...và được miêu tả trong Hình đồ như sau: “Lưu Bang còn dẫn theo một đám người, bao gồm Phàn Khoái khôi ngô, Chu Bột oai hùng, Lư Quán hào hoa phong nhã, còn có Hạ Hầu Anh nhanh nhẹn dũng mãnh”. Những nhân vật này trong lịch sử đã giúp Lưu Bang thành lập nên nhà Hán.',
      'Nhưng với sự xuất hiện của Lưu Khám, liệu lịch sử có thay đổi không khi hắn là người biết trước lịch sử, hiểu rõ thời thế, biết nắm bắt cơ hội, biết chiêu mộ những người tài hiệu lực cho mình. Trong thâm tâm hắn lại luôn có mối ác cảm với Lưu Bang, giữa hai người lại có mối thù riêng, và đặc biệt hắn không muốn một Đại Tần bị diệt vong sớm như trong lịch sử, bởi vậy đã tạo cho hắn và Lưu Bang trở thành hai chiến tuyến đối đầu nhau.',
      'Lưu Khám sẽ tạo nên một lịch sử riêng cho mình như nào?'
    ],
    view: '1,082,215',
    date: '17:57:05 06/06/2017',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - CƯỠNG HIẾP CẢ THÀNH PHỐ',
      'Chương 2 - NGƯỜI BỊ ĐÁNH DẤU',
      'Chương 3 - NỖI SỢ HÃI',
      'Chương 4 - QUỶ HÚT MÁU',
      'Chương 5 - BÁC SĨ',
      'Chương 6 - MÊ LỰC CỦA MÁU',
      'Chương 7 - CỐ ĐỂ QUÊN',
      'Chương 8 - VUI SƯỚNG HAY KHÔNG',
      'Chương 9 - LỘ DIỆN',
      'Chương 10 - NGÔI SAO 5 CÁNH TRÊN CÁNH CỬA',
      'Chương 11 - THÀNH HỒI ỨC',
      'Chương 12 - VỤ ÁN TẠI BỆNH VIỆN',
      'Chương 13 - BẢN NĂNG',
      'Chương 14 - LỌ HOA CỦA GRAYSON PERRY',
      'Chương 15 - MÊ HỒN TRẬN',
      'Chương 16 - SÁT THỦ CON SỐ',
      'Chương 17 - LỢN',
      'Chương 18 - THE YORKSHINE RIPPER',
      'Chương 19 - TÌNH YÊU LÀ GÌ',
      'Chương 20 - MÈO VÀ CHUỘT',
      'Chương 21 - 3+1+3',
      'Chương 22 - MÈO VÀ CHUỘT (2)',
      'Chương 23 - ĐÊM NOEL',
      'Chương 24 - ĐƯỜNG BƠI SỐ 6',
      'Chương 25 - PHÓNG 304',
      'Chương 26 - SƯ HUYNH',
      'Chương 27 - ĐẠI HIỆP SÔNG HÔ LAN',
      'Chương 28 - TẦNG TRÊN. ĐỊA NGỤC',
      'Chương 29 - VĨ THANH'
    ]
  },
  {
    id: 510,
    bookName: 'QUÂN HÔN',
    bookCover: 'https://img.dtruyen.com/public/images/large/quanhon35CI1HfDlo.jpg',
    rating: '7.7',
    language: 'Việt Nam',
    pageNo: 1210,
    author: 'Luật Nhi Loại Biệt',
    genre: 'lịch sử',
    content: '',
    readed: 72,
    description: [
      'Truyện lịch sử Quân Hôn nói về cô mở miệng nói như đùa: "Chúng ta kết hôn đi!"',
      'Kết quả, người đàn ông đối diện thật sự đáp lại: "Tốt, vừa vặn hộ khẩu của bản thân tôi cũng mang theo"',
      'Vì thế, cô bị hắn lừa gạt, cầm theo hộ khẩu của mình đến cục dân chính.... kết hôn.',
      '_______',
      'Cố Niệm Hề cả đời bi thảm nhất, chính là bị bạn tốt dồn đến góc tường, cướp mất đi bạn trai của mình trên thành phố xa lạ này',
      'Nhìn bọn họ hừng hực khí thế đại chiến ở trên giường, một ý niệm tà ác hiện ra trong đầu cô: Lão nương nhất định sẽ chuyển hộ khẩu của ta đến nhà của ngươi.',
      'Nếu không đảm đương nổi lão bà, kia làm người chị dâu cũng không ngại!',
      'Nhưng thật không ngờ lão công được cô chọn trúng, lại chính là một con hồ ly giảo hoạt, vô cùng phúc hắc nha.',
      'Mời bạn đọc khám phá tựa truyện hay.',
      ''
    ],
    view: '786,967',
    date: '12:55:41 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Tẩy Trần',
      'Chương 2 - Từ Chối May',
      'Chương 3 - Tìm Mèo',
      'Chương 4 - Vụ Giết Người Tàn Bạo',
      'Chương 5 - Bóng Ma',
      'Chương 6 - Phát Điên',
      'Chương 7 - Giấc Mộng Đêm',
      'Chương 8 - Cõng Ma',
      'Chương 9 - Tiếng Cười',
      'Chương 10 - Thăm Bệnh',
      'Chương 11 - Gọi Hồn',
      'Chương 12 - Đào Mộ',
      'Chương 13 - Mượn Sách',
      'Chương 14 - Ca Băng',
      'Chương 15 - Tòa Cao Ốc',
      'Chương 16 - Nghe Hát Trong Vườn',
      'Chương 17 - Ăn Cắp Vải',
      'Chương 18 - Rung Động',
      'Chương 19 - Tiểu Ngư',
      'Chương 20 - Nguồn Gốc Lời Nguyền',
      'Chương 21 - Thay Đổi',
      'Chương 22 - Trở Về Thành Phố',
      'Chương 23 - Nhập Hồn',
      'Chương 24 - Tiếc Thương Người Đã Khuất',
      'Chương 25 - Chờ Đợi (Hết)'
    ]
  },
  {
    id: 511,
    bookName: 'DẠ THIÊN TỬ',
    bookCover: 'https://img.dtruyen.com/public/images/large/dathientuuIXGj6aaJl.jpg',
    rating: '6.1',
    language: 'Việt Nam',
    pageNo: 1202,
    author: 'Nguyệt Quan',
    genre: 'lịch sử',
    content: '',
    readed: 29,
    description: [
      'Bên cạnh tựa truyện đình đám Đế Quốc Cuồng Lan, Dạ Thiên Sử tuy không nổi trội về bề ngoài nhưng càng chiêm nghiệm càng dễ gây nghiện, càng khai thác bên trong càng nhận ra những điểm hấp dẫn, đặc sắc đầy bất ngờ.',
      '***',
      'Có người nói nơi này là âm tào địa phủ. Loại người như chúng ta đúng là quỷ tốt trong điện Diêm La, nói linh tinh, rõ ràng là linh tinh mà! Đây là người không biết về chúng ta vu oan chúng ta! Loại thành kiến và hiểu lầm này khiến chúng ta mệt nhọc, khiến người tận trung với cương vị công tác vô cùng đau đớn.',
      'Người nói chuyện mặc một bộ đồ nha sai màu xanh nhạt, trên đầu đội mũ vải có vẻ lớn hơn đầu hắn một chút, bên hông thắt một cái đai vải đỏ cổ xưa, dưới chân là một đôi giày ống bề ngoài đen trắng không quá vừa chân, trông rõ ràng là một tên lính canh ngục.....',
      '***',
      'Hắn vốn dòng dõi quan chức nhưng không phải vương hầu, xuất thân thế gia nhưng lại không phải cao môn. Chỉ là một kẻ đứng đầu Lục Phiến Môn. Hắn vốn định cứ thế ôm cái bát sắt từ thời tổ tiên truyền xuống, không ngờ gặp phải một tên lừa đảo dẫn tới một nơi khác. Chuyến đi này, một đi khó có ngày về.',
      'Hắn tự nhận nghĩa bạc vân thiên, thái độ làm người tứ hải, là một bằng hữu có thể gửi gắm cả thê tử cho. Nhưng chỗ tới, tất cả cao phụ quý nữ đều trốn trong khuê phòng không gặp người. Hắn tự xưng tính cách thuần lương, dám giúp mọi người làm điều tốt...',
      '***',
      'So với truyện lịch sử khác, điển hình như Cẩm Y Vệ thì Dạ Thiên Tử mang nội dung sâu sắc, có chiều sâu, mang hơi hướm quân sự. Mời bạn đọc theo dõi những diễn biến tiếp theo của câu chuyện và hứa hẹn sẽ mang đến cái kết đầy thuyết phục.',
      ''
    ],
    view: '786,689',
    date: '09:35:10 02/10/2019',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Phu Quân Hung Mãnh (1)',
      'Chương 2 - Phu Quân Hung Mãnh (2)',
      'Chương 3 - Người Lạ Chớ Đến Gần (1)',
      'Chương 4 - Người Lạ Chớ Tới Gần (2)',
      'Chương 5 - Ly Long (*) Huyết Ngọc (1)',
      'Chương 6 - Ly Long Huyết Ngọc (2)',
      'Chương 7 - Ly Long Huyết Ngọc (3)',
      'Chương 8 - Ly Long Huyết Ngọc (4)',
      'Chương 9 - Tăng Ngọc Chương (1)',
      'Chương 10 - Bố Gặp Chuyện',
      'Chương 11 - Chồng Quỷ Ra Tay',
      'Chương 12 - Âm Khí Thấm Vào Xương Tủy',
      'Chương 13 - Tình Nghĩa Bạn Thân',
      'Chương 14 - Đừng Đi Giày Cao Gót',
      'Chương 15 - Bức Tượng Đồng Nhức Mắt',
      'Chương 16 - Tà Đạo Hại Người',
      'Chương 17 - Mặt Quỷ Lượn Lờ',
      'Chương 18 - Một Đêm Ba Thi Thể',
      'Chương 19 - Căn Phòng Chết',
      'Chương 20 - Tà Sư (1)',
      'Chương 21 - Tà Sư (2)',
      'Chương 22 - Sự Phẫn Nộ Lạnh Băng',
      'Chương 23 - Rỉ Thai',
      'Chương 24 - Nữ Quỷ Nhà Giàu (1)',
      'Chương 25 - Nữ Quỷ Nhà Giàu (2)',
      'Chương 26 - Tôi Là Khôn Đạo?',
      'Chương 27 - Tà Linh Bất Diệt',
      'Chương 28 - Lần Đầu Đến Thành Phố Quỷ',
      'Chương 29 - Nơi Tụ Âm Khí Giữ Thi Thể (1)',
      'Chương 30 - Nơi Tụ Âm Khí Giữ Thi Thể (2)'
    ]
  },
  {
    id: 512,
    bookName: 'NHẠC PHI DIỄN NGHĨA',
    bookCover: 'https://img.dtruyen.com/public/images/large/nhacphidiennghiaymWESwft8l.jpg',
    rating: '4.7',
    language: 'Việt Nam',
    pageNo: 680,
    author: 'Mộng Bình Sơn',
    genre: 'lịch sử',
    content: '',
    readed: 12,
    description: [
      'Truyện mới – Nhạc Phi Diễn Nghĩa thuộc loại lịch sử, quân sự, được chép và lưu hành vào đời nhà Thanh, nhưng nội dung truyện lại xảy ra ở đời nhà Tống, cách đấy hơn nửa thế kỷ. Đó là thời kỳ suy vong của dân tộc Hán.',
      'Bộ truyện Nhạc Phi nhằm đề cao lòng yêu nước của người dân Trung Hoa, đồng thời cũng bóc trần bộ mặt thật của các triều đại phong kiến thối nát, ươn hèn.',
      'Cũng như nhiều bộ sử trung ~ ung Quốc lúc ấy, Nhạc Phi cũng có nhiều yếu tố thần thoại, mê tính được lưu truyền, người đời sau sưu tập, tu chỉnh thành bộ truyện mạch lạc.',
      'Tuy nhiên, điều đó không làm mất đi giá trị nhân văn của tác phẩm, mà càng làm cho ta thấy rõ chế độ khắc nghiệt của triều đại phong kiến Tr ung Quốc, cố tình bưng bí sư thật, trấn áp tiếng nói chân chính của nhân dân. Để lên án những ông vua tham tàn, bất công thời đó, tất nhiên tác giả phải dùng các yếu tố thần thoại để che mắt vua quan, và cũng để dễ dàng châm biếm, đả kích bọn chúng.',
      "Nhạc Phi truyền được chép và lưu hành vào cuối đời Mãn Thanh đã gián tiếp lên án triều đình đương thời. Tuy cách xa nhau hơn nửa thế kỷ, nhưng hai triều đại 'Tống - Mãn này có những nét tương đồng.",
      'Lấy chuyện xưa để nói chuyện nay, truyện Nhạc Phi đã gửi một thông điệp yêu nước tới nhân dân Trung Quốc lúc đó.',
      'Nhạc Phi đã phần nào làm thỏa mãn lòng yêu nước chân chính của nhân dân, nên nó vẫn là một bộ truyện có giá trị.',
      'Bên cạnh đó, Bút Vẽ Giang Sơn Mực Tô Xã Tắc và Đế Yến là hai tựa truyện có nội dung chuyên về quân sự và lịch sử mà bạn đọc có thể điểm qua.',
      ''
    ],
    view: '170,948',
    date: '10:48:27 12/01/2015',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - . Bóng Dừa Dưới Trăng',
      'Chương 2 - Con Vàng',
      'Chương 3 - Giấc Ngủ Trưa',
      'Chương 4 - Kẻ Vô Hình',
      'Chương 5 - Cái Võng',
      'Chương 6 - Ai Đưa Võng Đong Đưa',
      'Chương 7 - Cái Mũ Cối',
      'Chương 8 - Bà Hàng Xóm',
      'Chương 9 - Trắng Và Đen',
      'Chương 10 - Ánh Đèn Chớp Tắt',
      'Chương 11 - Mèo Đen',
      'Chương 12 - Truyền Nhân',
      'Chương 13 - Hứa Gia',
      'Chương 14 - Ánh Lửa Trong Đêm',
      'Chương 15 - Trứng',
      'Chương 16 - Cuộc Gặp Gỡ',
      'Chương 17 - Vắng Nhà',
      'Chương 18 - Ông Ba Bị',
      'Chương 19 - Ảo Giác',
      'Chương 20 - Ma Giấu',
      'Chương 21 - Đụng Độ Với Yêu Ma',
      'Chương 22 - Cuộc Chiến Âm Dương',
      'Chương 23 - Bình Minh Mới',
      'Chương 24 - Bàn Tính',
      'Chương 25 - Đi Đám Ma',
      'Chương 26 - Căn Nhà Gỗ',
      'Chương 27 - Truy Tìm Nguồn Gốc',
      'Chương 28 - Âm Dương Hỏa Khuyển',
      'Chương 29 - Quyết Định',
      'Chương 30 - Bày Trận'
    ]
  },
  {
    id: 513,
    bookName: 'ĐẠI HÁN ĐẾ QUỐC PHONG VÂN LỤC',
    bookCover: 'https://img.dtruyen.com/public/images/large/daihandequocphongvanlucR6I5hx48iN.jpg',
    rating: '8.2',
    language: 'Việt Nam',
    pageNo: 1111,
    author: 'Mãnh Tử',
    genre: 'lịch sử',
    content: '',
    readed: 84,
    description: [
      'Truyện Đại Hán Đế Quốc Phong Vân Lục là một khá đặc sắc được giới thiệu đến với độc giả, truyện không phải là một truyện võ hiệp li kì cũng chẳng phả là một truyện thần tiên đầy ảo mộng mà là một truyện lịch sử, quân sự logic, chân thực nhưng sống động. Dưới ngòi bút của Mãnh Tử từng tình tiết trở nên thu hút hơn, dễ dàng khiến độc giả chìm đắm vào không khí mang màu sắc cổ xưa này, cách dụng binh, giữ nước, điều hành mọi thứ trong quốc gia đáng để ta lưu tâm.',
      'Quyển sách lấy nguyên tắc “Xâm nhập lịch sử cùng nhảy ra lịch sử”, miêu tả tiến trình lịch sử rắc rối phức tạp cách đây hơn 1.800 năm cùng chiến tranh tranh bá mạnh mẽ hoành tráng. Tiểu thuyết lấy Báo tử Lý Hoằng thân phận nô lệ cứu ra đại soái Tiên Ti, trốn về Đại Hán, bắt đầu từ một sĩ tốt bình thường, lần lượt lập kì công, trở thành tướng quân, do đó ảnh hưởng toàn bộ lịch sử Hán mạt. Từ nhiều manh mối, nhiều góc độ, nhiều mặt, nhiều trình tự tái hiện những năm cuối Đông Hán, thay đổi diện mạo lịch sử cùng một đám nhân vật anh hùng vì thay đổi vận mệnh thiên hạ mà phấn đấu, công bố quy luật vận động phát triển của lịch sử.',
      'Lý Hoằng cùng bằng hữu chiến hữu là tình nghĩa, cùng kẻ địch là chém giết đẫm máu, cùng vài vị giai nhân tình cảm dây dưa, cùng toàn bộ vận mệnh Đại Hán liên hệ, hình thành một cơn lốc xoáy thật lớn, xoay chuyển toàn bộ cục diện thiên hạ.',
      'Quyển sách còn ở trên cảnh tượng cuộc sống xã hội Hán mạt tốn rất nhiều văn chương cùng tâm tư, từ cung đình đến chiến trường, từ đô thành đến hương dã, từ Đại Mạc đến Trung Nguyên, gắng đạt tới nguyên trạng của lịch sử; Từ đấu đá chính đàn đến giao phong sa trường, từ điển chương lễ nghi đến nhân tình phong tục, gắng thể hiện truyền thống văn hóa Đại Hán.',
      'Đọc truyện để chiêm nghiệm nhiều thứ, và rút ra những bào học cho bản thân thêm cả những triết lý sống. Đọc để hiểu hơn một chút về lịch sử, đọc và cùng đồng hành với chuyến phiêu lưu. Bạn cũng có thể theo dõi những truyện khác cùng thể loại như: Quốc Sắc Sinh Kiêu, Tiên Quốc Đại Đế,...',
      ''
    ],
    view: '442,086',
    date: '23:59:01 03/09/2015',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',   'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',   'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',   'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',   'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',   'Chương 10 - Chương 10',
      'Chương 11 - Chương 11', 'Chương 12 - Chương 12',
      'Chương 13 - Chương 13', 'Chương 14 - Chương 14',
      'Chương 15 - Chương 15', 'Chương 16 - Chương 16',
      'Chương 17 - Chương 17', 'Chương 18 - Chương 18',
      'Chương 19 - Chương 19', 'Chương 20 - Chương 20'
    ]
  },
  {
    id: 514,
    bookName: 'TRỞ VỀ NĂM 1981 Oº°',
    bookCover: 'https://img.dtruyen.com/public/images/large/trovenam1981o3o3mHmVnmQ.jpg',
    rating: '7.6',
    language: 'Việt Nam',
    pageNo: 1141,
    author: 'Tú Cẩm',
    genre: 'lịch sử',
    content: '',
    readed: 79,
    description: [
      'Bạn đang đọc truyện Trở về năm 1981 oº° của tác giả Tú Cẩm trên website đọc truyện online. Từ hiện tại năm 2010Chung Tuệ Tuệ nhận được nhiệm vụ bí mật quay trở về năm 1981 để thay đổi số phận của Kim Minh Viễn – tên tội phạm giết người hàng loạt nhưng có quá khứ vô cùng đáng thương – tránh cho nhân gian một trận tanh máu và cũng để cứu rỗi cuộc đời một con người. Nhờ có Tuệ Tuệ mà cậu bé ba tuổi mồ cô Minh Viễn đã có một mái ấm, được yêu thương và được dạy dỗ, , bánh xe số phận của cậu đã rẽ sang một hướng khác. Nhưng trong lúc theo đuổi một vụ án, Tuệ Tuệ đã bị thủ tiêu bằng một vụ tai nạn giao thông được dàn dựng. Minh Viễn hết sức đau khổ và quyết tâm theo đuổi thủ phạm đến cùng. Nhưng liệu biến cố này có lại đẩy Minh Viễn đến một thảm cảnh khác? Và liệu Tuệ Tuệ có thực sự đã “chết”?'
    ],
    view: '499,352',
    date: '02:53:40 07/01/2019',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Hai Người Chồng Ma',
      'Chương 2 - Kết Hôn Với Anh Ta Thì Phải Chết',
      'Chương 3 - Trả Hết Thù Mới Với Nợ Cũ',
      'Chương 4 - Bên Trong Lò Thiêu',
      'Chương 5 - Giấc Mơ Tối Hôm Qua Là Thật?',
      'Chương 6 - Nuôi Quỷ Nuôi Thi',
      'Chương 7 - Sống Phải Thấy Người Chết Phải Thấy Xác',
      'Chương 8 - Một Vợ Nhiều Chồng',
      'Chương 9 - Đạo Bào Của Bà Ngoại',
      'Chương 10 - Hương Hương Cứu Mạng',
      'Chương 11 - Xử Lý Thích Uý Nhiên',
      'Chương 12 - Thay Trời Hành Đạo',
      'Chương 13 - Chồng Ma Mau Cứu Em',
      'Chương 14 - Lưng Của Tôi Đau Quá',
      'Chương 15 - Chồng Ma Đừng Lấn Tới',
      'Chương 16 - Cậu Đừng Chạy Lung Tung',
      'Chương 17 - Con Gái Ma',
      'Chương 18 - Tuyết Nhi',
      'Chương 19 - Chạy Thoát Quan Trọng Hơn',
      'Chương 20 - Tôi Là Chấp Niệm Cuối Cùng Của Anh',
      'Chương 21 - Đi Đêm Lắm Có Ngày Gặp Ma',
      'Chương 22 - Dấu Ấn Của Ác Quỷ',
      'Chương 23 - Đốt Quần Áo Cho Tôi',
      'Chương 24 - Trang Điểm Cho Xác Chết',
      'Chương 25 - Chồng Ma Rất Mạnh Mẽ',
      'Chương 26 - Quỷ Chị Còn Không Sợ',
      'Chương 27 - Luyện Cương Thi Kinh Khủng',
      'Chương 28 - Cương Thi Chú Đặng',
      'Chương 29 - Không Chống Nổi',
      'Chương 30 - Lời Hứa Hôn Trước Khi Chết'
    ]
  },
  {
    id: 515,
    bookName: 'SỦNG PHI BÃI CÔNG HẰNG NGÀY',
    bookCover: 'https://img.dtruyen.com/public/images/large/sungphibaiconghangngayjk6A8emhQ1.jpg',
    rating: '10',
    language: 'Việt Nam',
    pageNo: 615,
    author: 'Trầm Ổ',
    genre: 'lịch sử',
    content: '',
    readed: 40,
    description: [
      'Truyện Sủng Phi Bãi Công Hằng Ngày của tác giả Trầm Ổ kể về Nghi phi là phi tần được sủng ái nhất, vô số người hâm mộ, ghen tị.',
      'Vừa có phong thái kiều mỹ, nhan sắc tuyệt diễm, lại còn được ưu ái như thế, bảo sao không khiến người ghen ghét.',
      'Thế nhưng hôm nay, Nghi phi trở nên khác thường.',
      'Nàng đã mơ một giấc mộng, một cơn ác mộng dự báo tương lai...',
      'Thái tử rơi đài, Tứ hoàng tử lên ngôi, Đức phi, người luôn đối đầu với nàng lại thành Thái Hậu.',
      'Ba nhi tử do nàng sinh, lão Ngũ không được coi trọng, tiểu Cửu bị giam giữ cho đến chết, tiểu Thập Nhất thì từ nhỏ đã đau ốm không dứt, sau đó cũng ra đi.',
      'Hoàng đế Khang Hi từng lời ngon tiếng ngọt sủng nàng vạn phần, đến tuổi già vẫn tiếp tục nạp thêm phi tần cung nữ, đem nàng quẳng ra sau đầu.',
      'Tình nghĩa đến thế, vậy mà trước khi băng hà, hắn còn cố ý dặn dò tân đế rằng nếu Nghi phi ương ngạnh bất kính, thì không cần bận lòng, cứ thẳng tay mà diệt!',
      'Cuối cùng nàng vì u buồn mà chết, kết cục thê lương.',
      'Nếu yêu thích truyện ngôn tình, bạn đừng bỏ lỡ Nhà Trọ Cực Phẩm hay Đích Nữ Vô Song.',
      ''
    ],
    view: '7,200',
    date: '17:01:34 11/10/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Phần Dẫn 1&2',
      'Chương 2 - Vụ Án Mưu Sát 405',
      'Chương 3 - Những Tiếng Bước Chân Trong Phòng Giải Phẫu',
      'Chương 4 - Bí Ẩn Xác Chết',
      'Chương 5 - Thực Và Ảo',
      'Chương 6 - Giữa Sống Và Chết',
      'Chương 7 - Xác Chết Biết Nói',
      'Chương 8 - Chuyện Ly Kỳ',
      'Chương 9 - Phỏng Vấn Người Chết',
      'Chương 10 - Ánh Trăng, Ánh Trăng Là Gì?',
      'Chương 11 - Bạn Tri Âm Hiếm Có',
      'Chương 12 - Ngọc Nát Và Ngói Lành',
      'Chương 13 - Chuẩn Đoán Bệnh Thần Kinh',
      'Chương 14 - Trôi Dạt',
      'Chương 15 - Nguồn Cơn Của Nỗi Khiếp Sợ',
      'Chương 16 - Người Đàn Bà Mặt Sẹo Và Uông Lan San',
      'Chương 17 - Ánh Trăng, Mặt Nát, Nhập Vai',
      'Chương 18 - Khắc Khoải Nhớ Nhung',
      'Chương 19 - Cuộc Phân Tích Hãi Hùng',
      'Chương 20 - Đúng Hay Là Sai',
      'Chương 21 - Ký Sự Bắt Ma',
      'Chương 22 - Không Sao Hiểu Nổi',
      'Chương 23 - 15h Ngày 7 Tháng 6',
      'Chương 24 - Sự Phản Bội Dịu Dàng',
      'Chương 25 - Định Mệnh Khó Thoát',
      'Chương 26 - Y Y',
      'Chương 27 - Bí Mật Bị Phơi Bầy',
      'Chương 28 - Những Cơn Mộng Du Kinh Hồn',
      'Chương 29 - Tai Họa Do Con Người Đem Đến',
      'Chương 30 - Đỏ Và Đen'
    ]
  },
  {
    id: 516,
    bookName: 'PHƯỢNG ẨN THIÊN HẠ',
    bookCover: 'https://img.dtruyen.com/public/images/large/phuonganthienhadyfBu7Rx6v.jpg',
    rating: '7.6',
    language: 'Việt Nam',
    pageNo: 1495,
    author: 'Nguyệt Xuất Vân',
    genre: 'lịch sử',
    content: '',
    readed: 94,
    description: [
      'Truyện Phượng Ẩn Thiên Hạ là một truyện mới gửi đến bạn đọc trên trang đọc truyện online, truyện khá đặc sắc và thu hút với nhiều tình tiết mới lạ không hề như những truyện viết về đề tài quân sự khác, truyện nhẹ nhàng lại thành thoát, dễ đivào lòng bạn đọc. Truyện cũng là truyện võng du, có kết hợp truyện lịch sử',
      'làm cho câu chuyện thêm phần sống động lại khiến độc giả thêm phần hào hứng.',
      'Rượu hợp cẩn đêm động phòng hoa chúc là một ly rượu độc, nói ra ai có thể tin? Không ngờ người hạ độc lại chính là phu quân của nàng, nói ra ai có thể tin! Nhưng chuyện như vậy đã xảy ra! Mà tất cả chỉ là mới bắt đầu.',
      'Đêm động phòng, nàng từ tân nương trở thành người bị chồng ruồng bỏ, trong nháy mắt trở thành công chúa hòa thân, bị đẩy đến phương Bắc rét lạnh, hoang dã. Vẫn là tân nương, chỉ là trước đây được người khác hâm mộ bao nhiêu, bây giờ cái nàng nhận được chỉ là sự thương cảm của người đời. Vì giữ lại tính mạng của một người con gái tài mạo song toàn, nàng đã trở thành một người có thể lợi dụng, dùng xong vứt bỏ.',
      'Chỉ là không có ai biết, cho tới bây giờ nàng cũng không phải là một người con gái yếu đuối tuân theo sự sắp đặt của người khác. Vận mệnh của nàng, chỉ có thể do một mình nàng nắm giữ trong tay.',
      'Liệu cái kết cho truyện sẽ ra sao, có chăng vẫn hay nói anh hùng không qua nổi ải mỹ nhân, cửa ai này liệu có dễ dàng chui lọt, mời bạn đón đọc truyện đặc sắc này và theo dõi những truyện khác cùng thể loại như: Ở Rể ( Chuế Tế ), Tử Xuyên Tam Kiệt,...'
    ],
    view: '1,245,923',
    date: '16:08:04 05/03/2015',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Bị Người Xâm Nhập',
      'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',
      'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',
      'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',
      'Chương 8 - Chương 8',
      'Chương 9 - Đây Là Nhà Tôi',
      'Chương 10 - Chương 10',
      'Chương 11 - Chương 11',
      'Chương 12 - Chương 12',
      'Chương 13 - Chương 13',
      'Chương 14 - Chương 14',
      'Chương 15 - Chương 15',
      'Chương 16 - Chương 16',
      'Chương 17 - Chương 17',
      'Chương 18 - Chương 18',
      'Chương 19 - Chương 19',
      'Chương 20 - Chương 20',
      'Chương 21 - Chương 21',
      'Chương 22 - Chương 22',
      'Chương 23 - Chương 23',
      'Chương 24 - Chương 24',
      'Chương 25 - Chương 25',
      'Chương 26 - Chương 26',
      'Chương 27 - Chương 27',
      'Chương 28 - Chương 28',
      'Chương 29 - Chương 29',
      'Chương 30 - Chương 30'
    ]
  },
  {
    id: 517,
    bookName: 'PHONG LƯU',
    bookCover: 'https://img.dtruyen.com/public/images/large/phongluuNmFLJW2oB6.jpg',
    rating: '6.6',
    language: 'Việt Nam',
    pageNo: 1464,
    author: 'Ngũ Lang Thúc',
    genre: 'lịch sử',
    content: '',
    readed: 9,
    description: [
      'Bạn đang đọc truyện Phong Lưu của tác giả Ngũ Lang Thúc trên trang đọc truyện online.Một thanh niên mạnh khỏe xuyên việt tới thời Đường, tán đệ nhất mỹ nữ Dương Quý Phi!E hèm, thể loại truyện không hẳn lịch sử, cũng không hẳn kiếm hiệp, có phần sắc hiệp. Lâu lâu cả nhà đổi món ăn cho đỡ nhạt miệngNét chính của truyện thì, hèm hèm. Ít tu luyện, hậu cung nhiều. Ngắn gọn mà súc tích.'
    ],
    view: '696,152',
    date: '13:18:13 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Q.1 - Chương 1 - Công Trình 723',
      'Q.1 - Chương 2 - Đích Đến',
      'Q.1 - Chương 3 - Bộ Phim Số Không',
      'Q.1 - Chương 4 - Máy Bay Shinzan',
      'Q.1 - Chương 5 - Hang Động',
      'Q.1 - Chương 6 - Chia Nhóm',
      'Q.1 - Chương 7 - Manh Mối',
      'Q.1 - Chương 8 - Một Xác Chết',
      'Q.1 - Chương 9 - Bãi Đá Ngầm',
      'Q.1 - Chương 10 - Hi Sinh',
      'Q.1 - Chương 11 - Mẩu Giấy',
      'Q.1 - Chương 12 - Người Mới Đến',
      'Q.1 - Chương 13 - Viên Hỷ Lạc',
      'Q.1 - Chương 14 - Một Người Điên',
      'Q.1 - Chương 15 - Lồng Sắt',
      'Q.1 - Chương 16 - Quỷ Nước',
      'Q.1 - Chương 17 - Cửa Sắt',
      'Q.1 - Chương 18 - Lũ Dâng',
      'Q.1 - Chương 19 - Cứu Viện',
      'Q.1 - Chương 20 - Nghỉ Ngơi Và Chỉnh Đốn',
      'Q.1 - Chương 21 - Đối Tượng Cứu Viện Thực Sự',
      'Q.1 - Chương 22 - Máy Bay Hạng Nhẹ',
      'Q.1 - Chương 23 - Đội Quân Vô Danh',
      'Q.1 - Chương 24 - Sóng Điện Vĩnh Cửu',
      'Q.1 - Chương 25 - Mẩu Giấy Thứ Hai',
      'Q.1 - Chương 26 - Lọn Tóc',
      'Q.1 - Chương 27 - Đỉa',
      'Q.1 - Chương 28 - Shinzan Dưới Dòng Nước',
      'Q.1 - Chương 29 - Thám Hiểm “Shinzan”',
      'Q.1 - Chương 30 - Còi Báo Động Phòng Không'
    ]
  },
  {
    id: 518,
    bookName: 'ĐẾ YẾN',
    bookCover: 'https://img.dtruyen.com/public/images/large/deyenkQyAEF58ix.jpg',
    rating: '6.2',
    language: 'Việt Nam',
    pageNo: 917,
    author: 'Mặc Vũ',
    genre: 'lịch sử',
    content: '',
    readed: 7,
    description: [
      'Truyện Đế Yến là một khá đặc sắc được giới thiệu đến với độc giả, truyện không phải là một truyện võ hiệp li kì cũng chẳng phả là một truyện thần tiên đầy ảo mộng mà là một truyện lịch sử, quân sự logic, chân thực nhưng sống động. Dưới ngòi bút của Mặc Vũ từng tình tiết trở nên thu hút hơn, dễ dàng khiến độc giả chìm đắm vào không khí mang màu sắc cổ xưa này, cách dụng binh, giữ nước, điều hành mọi thứ trong quốc gia đáng để ta lưu tâm.',
      'Tác phẩm này nói về Cẩm Y vệ thời Minh Thành Tổ Vĩnh Lạc đế Chu Lệ. Thông qua tác phẩm, những vụ kỳ án thời Minh Thành Tổ được khắc họa hết sức sống động. Những âm mưu chốn cung đình, những dục vọng quyền lực, những thế lực trong bóng tối thông qua bước chân điều tra của Cẩm Y vệ Thu Trường Phong dần dần lộ diện ra ngoài ánh sáng...',
      'Cuối con phố dài, bỗng nhiên một đội người chạy ra đến, bước nhanh như sấm, giây lát đã đến trước mặt người dân đó. Đội người đó không ngoại lệ, thân phi ngư phục, đai lung tú xuân đao, vẻ mặt lạnh lùng',
      'Dân chúng đầu đường không nhịn được ngồi xổm xuống, cúi đầu vẻ mặt sợ hãi kinh dị. Người ở kinh thành, người không biết hoàng đế rất nhiều nhưng không nhận ra đám người này tuyệt đối không có. Đội người tới đó chính là thiên tử thân binh hiển hách uy danh của kinh thành - Cẩm Y vệ',
      'Đọc truyện để chiêm nghiệm nhiều thứ, và rút ra những bào học cho bản thân thêm cả những triết lý sống. Đọc để hiểu hơn một chút về lịch sử, đọc và cùng đồng hành với chuyến phiêu lưu. Bạn cũng có thể theo dõi những truyện khác cùng thể loại như: Quốc Sắc Sinh Kiêu, Tiên Quốc Đại Đế,...'
    ],
    view: '835,042',
    date: '12:31:55 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Nhật Ký Của Jonathan Harker (Viết Tốc Ký)',
      'Chương 2 - Nhật Ký Của Jonathan Harker(Tiếp Theo)',
      'Chương 3 - Những Bức Thư',
      'Chương 4 - Đọc Nhật Ký Của MINA MURRAY & BÁC SĨ SEWARD',
      'Chương 5 - BẢN CẮT TỪ TỜ \\"THE DAILYGRAPH\\", NGÀY 8 THÁNG 8',
      'Chương 6 - Nhật Ký Của Mina Murray.',
      'Chương 7 - THƯ CỦA MINA HARKER GỬI LUCY WESTENRA.',
      'Chương 8 - THƯ CỦA BÁC SĨ SEWARD GỬI HON ARTHUR HOLMWOOD',
      'Chương 9 - NHẬT KÝ CỦA LUCY WESTENRA',
      'Chương 10 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 11 - Chương 11',
      'Chương 12 - Nhật Ký Của Mina Harker',
      'Chương 13 - Chương 13',
      'Chương 14 - Chương 14',
      'Chương 15 - Chương 15',
      'Chương 16 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 17 - Nhật Ký Của Jonathan Harker',
      'Chương 18 - Chương 18',
      'Chương 19 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 20 - Chương 20',
      'Chương 21 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 22 - NHẬT KÝ GHI ÂM CỦA BÁC SĨ SEWARD',
      'Chương 23 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 24 - NHẬT KÝ CỦA BÁC SĨ SEWARD',
      'Chương 25 - NHẬT KÝ CỦA MINA HARKER'
    ]
  },
  {
    id: 519,
    bookName: 'THUỶ HỬ TRUYỆN',
    bookCover: 'https://img.dtruyen.com/public/images/large/thuyhutruyen6Fa2sv7oxf.jpg',
    rating: '6.2',
    language: 'Việt Nam',
    pageNo: 1158,
    author: 'Thi Nại Am',
    genre: 'lịch sử',
    content: '',
    readed: 87,
    description: [
      'Bạn đang đọc truyện Thuỷ Hử Truyện của tác giả Thi Nại Am trên website đọc truyện online.',
      'Về đời nhà Tống, Triết Tôn Hoàng Đế làm vua, cách với đời của vua Nhân Tôn Hoàng Đế đã xa; Giữa phủ Khai Phong, ở ngay Đông Kinh, thuộc khu của thành Tuyên Vũ Quân nảy nồi một tên con nhà lông bông mất dạy, là dòng họ Cao, bày hàng đứng vào thứ hai.',
      'Anh ta thuở nhỏ không chịu làm ăn, chỉ thích múa thương đánh gậy, và đá cầu rất giỏi, bởi thế trong Kinh Sư gọi anh ta là Cao Cầu, đổi gọi Cao Nhị. Đoạn chữ cầu có chữ Mao là lông ở bên cạnh, sau phát tích khá lên, mới đổi chữ Cầu có chữ Nhân là người đứng cạnh.'
    ],
    view: '1,113,467',
    date: '08:11:57 11/09/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',   'Chương 2 - Chương 2',
      'Chương 3 - Chương 3',   'Chương 4 - Chương 4',
      'Chương 5 - Chương 5',   'Chương 6 - Chương 6',
      'Chương 7 - Chương 7',   'Chương 8 - Chương 8',
      'Chương 9 - Chương 9',   'Chương 10 - Chương 10',
      'Chương 11 - Chương 11', 'Chương 12 - Chương 12',
      'Chương 13 - Chương 13', 'Chương 14 - Chương 14',
      'Chương 15 - Chương 15', 'Chương 16 - Chương 16',
      'Chương 17 - Chương 17', 'Chương 18 - Chương 18',
      'Chương 19 - Chương 19', 'Chương 20 - Chương 20',
      'Chương 21 - Chương 21'
    ]
  },
  {
    id: 523,
    bookName: 'ĐÔNG A NÔNG SỰ',
    bookCover: 'https://img.dtruyen.com/public/images/large/donganongsu49uaeRYXGn.jpg',
    rating: '9.5',
    language: 'Việt Nam',
    pageNo: 709,
    author: 'Nam Sơn',
    genre: 'lịch sử',
    content: '',
    readed: 41,
    description: [
      'Truyện Đông A Nông Sự của tác giả Nam Sơn kể về một kỹ sư nông nghiệp vô tình lạc về triều Trần. Cám cảnh ngày ăn một bữa, 16 gia đình chung một con trâu. Hắn sẽ làm gì để hoàn thành ước mơ nuôi sống 4 triệu người Đại Việt khi đó. Đồng thời giúp Hào khí Đông A không còn là cái danh nhất thời, mỗi lần đánh bại quân Nguyên là đất nước không còn nổi mấy hạt gạo.',
      'Trên lịch sử, nhãn quang của Lý Thái Tổ chiếu rọi sử sách nước ta nghìn năm, Thái uý Lý công đồ thành Ung Châu, Khâm Châu chính là thời đại làm người ta nhung nhớ. Sự diệt vong của họ ít nhiều làm con cháu nuối tiếc, quốc gia yếu thì sập, chỉ có Nhà Lý mạnh quá tự sập. Vậy làm sao để nhà Trần và mãi mãi về sau không bao giờ phải phụ thuộc trung nguyên nữa?',
      'Nếu yêu thích truyện lịch sử, bạn có thể đọc thêm Tiêu Dao Tiểu Thư Sinh hoặc Bảy Viễn Thủ Lĩnh Bình Xuyên'
    ],
    view: '2,250',
    date: '17:08:28 11/10/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - Chương 1',
      'Chương 2 - Quyển Sách Dán Ảnh Của Tu Sĩ Alberic',
      'Chương 3 - Những Trái Tim Bị Mất',
      'Chương 4 - Bức Tranh In Khắc',
      'Chương 5 - Cây Tần Bì',
      'Chương 6 - Con Số 13',
      'Chương 7 - Bá Tước Magnus',
      'Chương 8 - Còi Ơi, Ta Sẽ Đến Với Mi, Chú Bé Của Ta',
      'Chương 9 - Kho Tàng Của Tu Viện Trưởng Thomas',
      'Chương 10 - Một Chuyện Ở Trường Học',
      'Chương 11 - Vườn Hồng',
      'Chương 12 - Luận Văn MIDDOTH',
      'Chương 13 - Quăng Các Chữ Runes [1]',
      'Chương 14 - Ghế Ngồi Hát Thành Ca Ở Nhà Thờ Barchester',
      'Chương 15 - Khu Đất Có Rào Martin',
      'Chương 16 - Ông Humphreys Và Địa Sản Thừa Kế',
      'Chương 17 - Nơi Cư Ngụ Ở Whitminster',
      'Chương 18 - Nhật Ký Của Ông Poynter',
      'Chương 19 - Một Giai Đoạn Của Lịch Sử Nhà Thờ Lớn',
      'Chương 20 - Chuyện Một Mất Tích Và Một Xuất Hiện',
      'Chương 21 - Hai Bác Sĩ',
      'Chương 22 - Ngôi Nhà Búp Bê Bị Ma Ám',
      'Chương 23 - Quyển Sách Kinh Bất Thường',
      'Chương 24 - Mốc Địa Sản Của Láng Giềng',
      'Chương 25 - Cảnh Nhìn Từ Một Ngọn Đồi',
      'Chương 26 - Cảnh Cáo Kẻ Tò Mò',
      'Chương 27 - Giải Trí Một Buổi Chiều',
      'Chương 28 - Có Một Người Sống Bên Cạnh Nghĩa Trang Nhà Thờ',
      'Chương 29 - Chuột',
      'Chương 30 - Chương 30'
    ]
  },
  {
    id: 533,
    bookName: 'ĐẠI TÙY QUỐC SƯ',
    bookCover: 'https://img.dtruyen.com/public/images/large/daituyquocsuAuLid7A.jpg',
    rating: '8.4',
    language: 'Việt Nam',
    pageNo: 1079,
    author: 'Nhất Ngữ Phá Xuân Phong',
    genre: 'lịch sử',
    content: '',
    readed: 47,
    description: [
      'Main nhặt được một vị Cóc tinh trọng thương, bị nó lừa bái sư nhưng lão yêu luôn muốn hại chết main, mà kết quả lại luôn là trời xui đất khiến thành tựu cho sự “Ngây thơ” của nhân vật chính nhà ta.',
      'Truyện viết nhẹ nhàng, tấu hài chủ yếu ở con Cóc yêu, kết hợp lịch sử, võ hiệp, tu tiên.',
      'Tóm lại, truyện hay, hài hước, nhẹ nhàng, thích hợp với tất cả mọi người!!'
    ],
    view: '17,150',
    date: '16:56:11 10/10/2021',
    backgroundColor: 'rgba(247,239,219,0.9)',
    navTintColor: '#000',
    chapter: [
      'Chương 1 - COLDWATER, MAINE MƯỜI BỐN THÁNG TRƯỚC',
      'Chương 2 - BÃI BIỂN DELPHIC, MAINE NGÀY NAY',
      'Chương 3 - Chương 2',
      'Chương 4 - Chương 3',
      'Chương 5 - Chương 4',
      'Chương 6 - Chương 5',
      'Chương 7 - Chương 6',
      'Chương 8 - Chương 7',
      'Chương 9 - Chương 8',
      'Chương 10 - Chương 9',
      'Chương 11 - Chương 10',
      'Chương 12 - Chương 11',
      'Chương 13 - Chương 12',
      'Chương 14 - Chương 13',
      'Chương 15 - Chương 14',
      'Chương 16 - Chương 15',
      'Chương 17 - Chương 16',
      'Chương 18 - Chương 17',
      'Chương 19 - Chương 18',
      'Chương 20 - Chương 19',
      'Chương 21 - Chương 20',
      'Chương 22 - Chương 21',
      'Chương 23 - Chương 22',
      'Chương 24 - Chương 23',
      'Chương 25 - Chương 24'
    ]
  }
]

